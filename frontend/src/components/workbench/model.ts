...
export interface WorkbenchStep {
  ...
  /**
   * 저장되지 않은 삽입인가 (009 FR-310).
   *
   * **왜 이 표식이 필요한가.** 세션 없는 편집에서는 모든 행의 결말이 「미실행」이므로 방금
   * 넣은 Step 과 저장된 Step 이 구별되지 않는다. 일시정지 세션에서는 구별할 수 있다 —
   * 이미 돌아간 Step 은 결말 표식을 갖고 방금 넣은 것은 갖지 않는다. 그래서 이 표식은
   * 편집 국면에서만 참이 된다.
   *
