...
- [ ] T156 `specs/001-interactive-ai-test-builder/checklists/design-review.md` 의 미체크 55항목을 리뷰어가 검토하고 결과를 반영한다 **미수행 — 리뷰어가 해야 한다.** 구현자가 자기 구현을 체크하면 리뷰가 아니라 자기 확인이 된다. 65항목이 미체크로 남아 있다. **검토 준비 완료 (2026-09-04)** — 진행 안내 `docs/review/design-review-guide.md`(판정 기준·항목 분류·4회 세션 분할·기록 절차), 근거 위치표 `docs/review/design-review-pointers.md`(65항목 전부를 `파일:라인` 으로 해석, 미해결 0건, 생성기 `scripts/review_pointers.py`). **판정은 들어 있지 않다**
  ...
  > **002 라운드의 영향 (2026-09-04).** T155 의 측정(SC-001·002·004·005)은 이 라운드가
  > 성립시킨 흐름에 달려 있었다 — 측정 대상인 "기존 프로젝트 열기", "AI로 만들기",
  > "녹화 후 저장" 이 실제로 동작하지 않아 측정 자체가 불가능했다.
  > `specs/002-defect-fix-design-conformance/` 가 그 세 흐름을 고쳤으므로 이제 측정을
  > 진행할 수 있다. T156 의 디자인 리뷰도 002 의 8화면 전사 이후를 대상으로 해야 한다 —
  > 001 시점의 화면은 확정 디자인과 달랐다.
...
`/speckit-converge` (2026-09-03, Phase 4 완료 시점) 가 spec.md·plan.md·contracts·헌법 대비
코드 현재 상태를 점검해 찾은 잔여 작업이다. **Phase 5~10 의 미완료 작업은 여기 포함하지
않는다** — 그것은 이미 등록된 계획 작업이며 converge 가 발견한 갭이 아니다.

- [X] T159 탭 닫힘을 `close_tab` Step 으로 기록한다 per FR-030c (missing) — **HIGH**. `Recorder.record_tab_close` 는 구현돼 있으나 호출되지 않고, `SessionManager._on_tab_closed` 는 `tab_closed` 이벤트만 발행한다. 사용자가 브라우저 UI 로 탭을 닫으면 아무 Step 도 남지 않아 재실행이 녹화와 갈라지고, "팝업을 닫은 뒤 원래 탭 상태를 검증하는 흐름"(FR-030c)이 성립하지 않는다. `page.on("close")` 경로를 리코더에 연결하고, `test_multitab_record.py` 에 `close_tab` Step 단언을 추가한다 (T051·T055 가 `[x]` 로 표시됐으나 이 배선과 단언이 없었다). **함께 고친 것**: `Recorder.watch_new_tab` 도 호출되지 않아 새 탭의 화면 이동이 기록되지 않았다 — 세션에 페이지 관찰자 훅을 두어 리코더가 새 탭마다 네비게이션·닫힘 감시를 붙인다. 클릭이 유발한 닫힘은 Step 으로 만들지 않고(FR-030b 와 같은 판정), 실행 시 `close_tab` 은 목표 상태 기준으로 판정한다(이미 닫혀 있으면 통과)
- [X] T160 WebSocket 계약과 구현의 불일치를 해소한다 per `contracts/websocket.md` (contradicts) — 경로(계약 `/ws/sessions/{sid}` vs 구현 `/api/sessions/{sid}/events`), `state_changed` 페이로드(계약의 `current_step_index` 없음, 계약에 없는 `active_tab` 있음), `tab_opened` 의 `title` 누락, `edit_warning` 이벤트 미발행(현재 REST 응답으로만 전달), 계약에 없는 `run_error`·`artifact_note` 발행. **계약을 고칠지 구현을 고칠지 한쪽으로 정한 뒤** `tests/contract/test_ws_events.py` 에 그 결정을 단언으로 고정한다 — 지금은 계약 문서를 읽고 프론트를 만들면 동작하지 않는다
- [X] T161 hover·drag 조작 충실도 per FR-023c (missing) — 리코더는 한글 IME(`compositionstart`/`end`)와 파일 선택 대화상자 경고는 처리하지만 hover·drag 는 Step 으로 기록하지 않는다(`pointerover` 는 후보 사전 수집 전용). hover 로만 열리는 드롭다운 메뉴는 재실행에서 재현되지 않는다. **Step 종류 추가는 원칙 I 에 따라 DSL 스키마 선행 변경을 요구하므로** 임의로 만들지 않는다 — ① 신규 Step 종류로 지원, ② 기존 6종으로 우회 가능함을 근거와 함께 기록, ③ FR-023c 의 범위를 축소(spec 변경) 중 하나를 골라 `plan.md` 또는 spec 에 남긴다. **결정 (2026-09-03, 사용자): ① hover·drag Step 종류를 추가한다.** 원칙 I 절차대로 DSL(`domain/step.py`) → JSON Schema → TS 타입 → 리코더 → 실행기 → 화면 순으로 반영했다. 기록 기준과 알려진 한계(아이콘만 있는 CSS hover 메뉴는 자동 감지 불가 → FR-036 으로 보완), 그리고 기록하지 않는 것(좌표 경로가 의미를 갖는 자유 드래그)을 `contracts/step-dsl.md` §hover 와 drag 에 남겼다
- [X] T162 손상된 실행 결과 파일을 "결과 없음" 으로 보고하지 않는다 per 헌법 §보안(명시적 오류 처리)·FR-087 (partial) — `ProjectRepository.read_result` 가 `except Exception: return None` 으로 삼켜, 결과 파일이 손상되면 API 가 `TEST_NOT_FOUND` "먼저 실행하세요" 를 돌려준다. 목록 조회에서 깨진 파일 하나가 전체를 막지 않게 하려는 원래 의도는 유지하되(`list_tests` 의 `problems` 와 같은 방식), **단건 조회는 손상 사유를 밝히도록** 경로를 나눈다
...
## Phase 13: Convergence (3회차)

`/speckit-converge` 3회차 (2026-09-03, Phase 5~10 완료 시점). spec.md 의 요구사항 136건·
성공 지표 12건, plan.md 의 결정, 헌법 원칙 5개 대비 코드 현재 상태를 점검했다.

**헌법 위반 0건.** 원칙 V의 Export 미구현은 릴리스 게이트 RG-1로 정식 등록된 이연이므로
위반이 아니다 (헌법 v1.1.0 Incremental delivery).

**네 건 모두 같은 모양이다 — 백엔드와 계약은 준비돼 있는데 화면이 잇지 않았다.** API·클라이언트
함수까지 있고 호출하는 곳만 없는 상태이므로, 백엔드 테스트로는 드러나지 않는다.

- [X] T167 목록에서 테스트 이름 변경·삭제를 할 수 있게 한다 per FR-007 (partial) — **HIGH**. `frontend/src/api/client.ts` 의 `tests.rename`·`tests.remove` 가 정의만 되어 있고 어디서도 호출되지 않는다. `TestList.tsx` 에 두 조작을 붙이고, **삭제는 되돌릴 수 없으므로 확인을 받는다** (FR-007). 확인 없이 지우면 사용자가 만든 자산이 한 번의 오클릭으로 사라진다 **구현**: 확인을 브라우저 `confirm` 이 아니라 목록 안에서 받는다 — 어느 테스트를 지우는지 눈으로 보면서 결정하게 하고, 무엇이 함께 지워지는지(정의 파일·실행 산출물)와 되살릴 방법(git)을 함께 알린다
- [X] T168 중지 전에 저장하지 않은 편집의 저장 여부를 확인한다 per FR-042 (partial) — **HIGH**. 서버는 이미 `POST .../stop` 응답과 세션 뷰에 `has_unsaved_changes` 를 실어 보내지만(T098) 프론트가 읽지 않는다. `Runner.tsx` 의 중지가 확인 없이 세션을 끝내므로, 일시정지 중 편집한 내용이 그대로 사라진다. 확인 시점은 **중지를 누른 직후**여야 한다 — 종료 후에는 저장할 대상이 없다 **구현**: 중지 시점에 확인한다. 종료 후에는 저장할 대상이 사라진다. "저장하고 계속 열어 두기" 를 함께 제공한다 — 저장만 하고 작업을 이어가려는 것이 흔한 의도인데, 저장 후 강제 종료하면 세션을 다시 만들어야 한다
- [X] T169 저장된 테스트의 정의를 실행하지 않고 볼 수 있게 한다 per FR-016·FR-019 (partial) — **HIGH**. 지금은 Step 목록과 상세를 보려면 재실행 세션을 시작해야 한다. `tests.get` 이 어디서도 쓰이지 않는 것이 그 증거다. 목록에서 테스트를 열어 Step 목록과 후보 우선순위 표(`LocatorPriorityTable`)를 보는 화면을 만든다. **"다시 집기"(FR-020)는 살아 있는 브라우저를 요구하므로 이 화면에 넣지 않는다** — 일시정지 세션에 남기고, 여기서는 보기와 이름·값 편집까지만 한다 **구현**: `pages/TestDefinition.tsx`. 세션을 만들지 않고 `GET /api/tests/{id}` 하나로 그린다 — 테스트가 그 사실(세션 엔드포인트 미호출)을 단언한다. **다시 집기는 넣지 않았다** — 살아 있는 브라우저를 요구하므로 일시정지 세션에 남기고, 왜 여기 없는지 화면에서 알린다
- [X] T170 결과 화면에서 실패한 Step 상세로 이동할 수 있게 한다 per FR-056 (partial) — MEDIUM. `RunResult.tsx` 는 `onEditStep` 을 이미 받지만 `App.tsx` 가 넘기지 않아 버튼이 그려지지 않는다. T169 의 정의 보기 화면이 생기면 그곳으로 연결한다 — 두 작업은 같은 목적지를 공유한다 **구현**: T169 의 정의 보기 화면으로 연결한다. 실패한 Step 을 지목해 들어가면 그 Step 의 후보 표가 펼쳐진 상태로 열린다

**Checkpoint**: 위 4건이 끝나면 spec·plan·contracts·헌법 대비 잔여 갭이 없다.
남는 것은 사람이 수행해야 하는 T155(수동 측정)·T156(리뷰)과 P2 릴리스 게이트 RG-1뿐이다.
