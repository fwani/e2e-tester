...
「대상이 없음」(N2)이다. 실제로 쓸 수 있는 것은 넷 — `test.setStartUrl` · `ai.compose` ·
`record.start` · `nav.back`. `ai.start` 는 지시문이 비어 있으면 못 누르므로 조건 C14 다.
...
| `step.*` | ○ | Step 이 0개다. O4 덮어쓰기가 이미 「Step 이 없습니다」로 ○ 로 만든다. 표에서도 ○ 로 두어 **자리가 남는 것**을 보장한다 (FR-260 — 목록이 0개여도 자리를 지킨다) |
...
| C14 | 지시문이 **비어 있지 않다** | 「지시문을 쓰면 시작할 수 있습니다」 → `ai.compose`. **2회차 추가** — 지금 `AiCompose.tsx:144` 의 `disabled={busy \|\| instruction.trim() === ""}` 가 하던 판정을 표로 옮긴다 |
