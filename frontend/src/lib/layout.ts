...
/**
 * ③-c 대화 자리 (016 · 2026-09-11 사용자 보고).
 *
 * > 「ai 대화가 미리보기 화면을 덮쳐서 아무것도 보이지 않는다」
 *
 * ## 무엇이 빠져 있었나 — **S-12 와 같은 형태다**
 *
 * 016 이 대화 패널을 좌측 열의 셋째 자리로 걸면서(`Workbench` 의 `leftExtra`) **배분을
 * 아무도 정하지 않았다.** 선언이 없는 flex 자식은 `0 1 auto` 이고 그 최소 높이는
 * 「내용 전체」다. 미러 자리는 `flex-1`(basis 0)이라 더 줄일 것이 없으므로, 대화가
 * 길어질 때 생기는 음의 여백을 **미러가 전부 먹는다** — 대화 차례가 쌓일수록 미러가
 * 0 에 가까워진다.
 *
 * 이 파일이 존재하는 이유가 그 결함이었다 (S-12). 자리가 둘에서 셋으로 늘 때 배분표만
 * 보고는 셋째가 빠진 것을 볼 수 없었으므로, 여기에 적어 둔다.
 *
 * ## 왜 표(`VERTICAL_SPLIT`)에 넣지 않는가
 *
 * 이 자리의 배분은 **국면에 따라 갈리지 않는다.** 갈리지 않는 판정을 표로 남기면 다음
 * 사람이 그것을 「국면마다 다를 수 있는 것」으로 읽는다 — 위에서 `DETAIL_PLACEMENT` 를
 * 「전부 겹침」으로 채우지 않고 없앤 것과 같은 규율이다.
 *
 * ## 왜 상한이 45% 가 아닌가
 *
 * `content` 의 45% 는 ③-b **하나**가 미러와 나눠 쓸 때의 값이다. 자리가 셋이 되면
 * 45% + 45% 가 미러를 10% 로 만든다. 세션이 있는 국면의 주 자리는 모두 미러이므로
 * (`PRIMARY_SLOT`), 대화는 그보다 작아야 한다 — ③-b 가 한 줄일 때 미러가 3분의 2 로
 * 남는 값이 3분의 1 이다. 두 상한의 합이 100% 를 넘지 않는 것을
 * `tests/ChatDoesNotCoverMirror.test.tsx` 가 센다.
 *
 * ## 여기에 `overflow-y-auto` 가 없는 이유
 *
 * 넘치는 것을 처리하는 자리가 패널 **안**이다. 대화 기록만 스크롤하고 입력칸은 늘
 * 보여야 한다 — 자리 전체를 스크롤 영역으로 두면 답이 길어질 때 쓸 칸이 스크롤
 * 아래로 사라진다. 그래서 이 상수는 「높이의 상한」만 정하고, 줄어드는 방법은
 * `ChatPanel` 이 자기 안에서 정한다 (`min-h-0` + 기록 영역의 스크롤).
 */
export const CHAT_SLOT_CLASS = "flex-none max-h-[33%] flex flex-col";
