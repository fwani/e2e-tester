...
|---|---|
| 정본 클래스 | **108** |
| 화면이 더는 쓰지 않는 것 (「완료」) | **65** |
| 아직 쓰는 것 | **43** |

착수 시 화면이 쓰던 것은 104개였다. 남은 43개는 대부분 **조건부 표현식
안**에 있어 기계 치환이 닿지 않았고, 부품(`ui/Chip`·`ui/Button` 등)으로 옮겨야 하는
것들이다 — 유틸리티로 풀면 같은 부품이 화면마다 다른 조합을 얻어 SC-010 이 깨진다.

**SC-009(미상 0건)는 달성했다.** 109행 전부 행선지가 적혀 있다. 「완료」로 가지 못한
것은 행선지를 몰라서가 아니라 옮기는 일이 남았기 때문이다.
