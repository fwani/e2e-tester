...
가시성은 **조작이 아니라 사유**에 붙는다 (`REASON_VISIBILITY`). 기준은 「지금 이 화면에서
곧바로 해소할 수 있는 전제인가」다.

| 사유 | 가시성 | 왜 |
|---|---|---|
| `NEEDS_SESSION` | **`keep`** | 편집 화면에서 이 기능의 존재를 알 수 있어야 한다. 해소 조작(`ai.rerecord`)을 가리킨다 |
| `USE_BLOCKED_ANSWER` | **`keep`** | 답이 어디로 가야 하는지 말한다 |
| `C16` (만든 것이 없다) | `hide` | 확정 버튼이 재녹화 아닌 화면에 상시로 떠 있을 이유가 없다 |
| `C17` (교체가 없다) | `hide` | 같음 — 버리기도 마찬가지 |
| `ALREADY_IN_SESSION` | `hide` | 세션 안에서 「AI 로 다시 만들기」는 찾을 일이 없는 조작이다. 남기면 모든 일시정지 화면에 잠긴 버튼이 하나 는다 |

`C7`(정의 편집 불가)은 기존 사유이며 이미 `keep` 이다 — 편집 국면의 `ai.rerecord` 가
그 덕에 보이되 잠긴다.

**이 표는 구현에서 고쳐졌다.** 초안은 조작 단위로 「`ai.rerecord`·`ai.chat` 은 keep」이라
적었는데, 가시성이 조작이 아니라 **사유**에 붙는다는 사실을 반영하지 못했다. 그 결과
일시정지 화면에 확정·버리기·재녹화 시작 버튼 셋이 잠긴 채 상시로 뜨는 상태가 됐고,
`CapabilityUI` 검사가 그것을 드러냈다.
