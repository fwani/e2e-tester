...
```bash
# 전환 전 코드를 git worktree 로 꺼내 빌드해 둔다 (`--compare` 가 알아서 하지만,
# 처음 한 번은 npm 설치와 빌드에 시간이 걸리므로 미리 돌려 두면 §3 이 빨라진다)
backend/.venv/bin/python scripts/design_compare_ba.py --baseline
```

> **기준선을 파일로 들고 다니지 않는다.** `--compare` 는 전·후를 **한 번에** 재므로
> 두 쪽이 같은 백엔드·같은 데이터·같은 브라우저를 쓴다는 것이 구조로 보장된다.
> `--baseline` 은 준비를 미리 해 두는 용도다.
...
### 1-4b. 한 요소에 같은 속성이 두 번 (가드 G-E · 계약 C-9)

```bash
cd frontend && npm test -- --run ClassConflict
```

`className` 의 순서는 승부를 정하지 않는다 — 산출 CSS 의 순서가 정한다. 부품이
조립하는 **조합 전부**를 보므로, 리터럴만 보는 검사가 영원히 못 보는 것을 잡는다.

**기대**: 통과. 실패하면 「나중에 적은 유틸리티가 지고 있다」는 뜻이고, 그것은 대개
화면이 적은 대로 그려지지 않는다는 뜻이다 (2026-09-11 「흰 버튼」이 그것이었다).
...
### 2-1b. 앱이 들이는 정본이 낡지 않음 (계약 C-10, 가드 G-A6)

```bash
node frontend/scripts/split-canon.mjs --check
cd frontend && npm test -- --run CanonSplit
```

앱은 정본 전체가 아니라 **뽑아낸 것**(`theme/tokens.app.css` — 변수와 요소 규칙만)을
들인다. 생성물이므로 정본이 바뀌면 다시 뽑아야 한다.

**기대**: 통과. 실패하면 정본을 고치고 다시 뽑지 않은 것이며, **화면은 옛 값으로
그려지는데 L1 대조는 정본을 읽으므로 통과한다** — 아무 검사도 실패하지 않는 회귀다.
고치는 법: `node frontend/scripts/split-canon.mjs`
...
cd frontend && npm test -- --run BeforeAfterParity
```

전환 전 코드를 `git worktree` 로 꺼내 빌드하고, **같은 백엔드·같은 데이터**로 화면
여덟 개를 열어 DOM 을 같은 순서로 훑으며 `getComputedStyle` 56개를 읽는다.
마크업 구조는 015 가 건드리지 않으므로 요소가 1:1 로 짝지어진다.

데이터는 `.l2-data/` 에 새로 심는다 — **사용자의 실제 프로젝트를 읽지도 쓰지도 않는다.**

**기대**: 의도되지 않은 불일치 0건. 실측 34,272칸.

불일치가 나오면 **각각이 의도된 것인지 판단한다.** 의도된 것이라면
`scripts/design_compare_ba.py` 의 `INTENDED` 에 **이유와 함께** 등록한다 — 이유 없이는
등록할 수 없으므로 그 목록은 예외가 아니라 판단이다.

**두 번째 명령을 빼먹지 않는다.** 화면 코드가 바뀌었는데 대조를 다시 돌리지 않으면
보고서의 「불일치 0」은 **지금 화면에 대한 말이 아니다.** `BeforeAfterParity` 가 소스
digest 로 그것을 잡고, 대조 범위(화면 8 · 속성 56 · 3만 칸)가 조용히 줄어드는 것도
함께 막는다 — 범위가 줄면 0 은 쉬워진다.

### 이 대조가 덮지 못하는 것

| 덮지 못함 | 왜 | 어디서 보나 |
|---|---|---|
| 작업대의 일곱 국면 | 실제 브라우저 세션이 있어야 들어간다 | 사람 (§4) |
| Step 상세 겹침 · 토스트 층 | 같음 | 사람 (§4) |
| 결과 · 편집 · 미러 화면 | 같음 | 사람 (§4) |
| hover · focus · 비활성 | 정적인 화면만 잰다 | `InteractionStates`·`FocusRing` 가드 + 사람 |
| 「기존 프로젝트 열기」 | 사용자의 홈 디렉터리를 그린다 — 보고서에 개인 경로가 남고 기계마다 다르다 | 사람 |

**덮지 못한 것이 어디인지가 남아야** 「불일치 0」이 무엇에 대한 말인지 읽을 수 있다
(FR-011).
