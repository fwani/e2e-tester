...
### 2026-09-16 · 대화상자 셋을 Base UI 로 (T104)

| § | 확인 | 결과 |
|---|---|---|
| 1-1 | 타입 검사 · 테스트 · 단언 수 | `tsc` 통과 · **1419 / 1419 통과** (파일 118) · 단언 **2276** (+2 · 상세 판 초점 검사 둘) · 무른 단언 **639** · 건너뜀 0 |
| 2 | 화면 깨짐 순회 | 76회 · **등록되지 않은 검출 0** · 닿지 못함 0 · 허용 28(사유 있음) |
| 3 | L2 대조 | **대조 34272칸** · 불일치 0 · 의도된 차이 188 |
| 4 | 번들 | **아직 판정하지 않는다** — `radix-ui` 가 T107 까지 함께 있다 |
| H | 사람 판정 | 그대로 — H-9·H-10 미실행 |

**가림막이 `Backdrop`+`Viewport` 로 갈리고 상세 판에 호스트 상자가 하나 생겼는데도 L2 에 등록할 차이가
없었다** — 그 여덟 화면에는 창이 떠 있지 않기 때문이다. 창이 뜬 화면은 순회가 본다(`test-list-bulk-confirm`·
`edit-delete-confirm`), 그쪽도 검출 0 이다.

**이 회차가 잡은 것**: 이식이 회귀를 하나 만들었고 `AuthoringParity` 가 잡았다 — 상세 판이 **옆 칸에 치던
글자를 가로챘다**(「장바구니에 담아」를 쳤는데 `'장'` 만 남았다). HEAD 에서 14/14 통과함을 확인해 내가
깨뜨린 것임을 먼저 못 박고 고쳤다. 고치는 동안 「열리면 초점이 판 자체로 간다」를 **아무 검사도 붙잡고
있지 않다**는 것이 드러나 검사 둘을 새로 넣었다 (test-ledger 「가드가 잡은 것」 넷째 줄).
