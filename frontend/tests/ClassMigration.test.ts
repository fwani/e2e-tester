...
import { canonClasses } from "./helpers/tailwind";
...
describe("G-D — 대응표와 코드가 일치한다", () => {
  ...
  it("「완료」로 적힌 클래스는 정본에서 사라졌다", () => {
    // 부품을 만들고 옛 정의를 남기면 두 체계가 공존한 채 굳는다 —
    // 007 이 겪은 「전사(轉寫)」 실패의 형태다 (data-model.md 상태 전이).
    const lying = table.filter((r) => r.done && canon.has(r.cls)).map((r) => `  .${r.cls}`);
    expect(
      lying,
      "표는 「완료」인데 `tokens.css` 에 정의가 남아 있다.\n" +
        "부품만 만들고 옛 정의를 남기면 두 체계가 공존한 채 굳는다.\n" +
        lying.join("\n"),
    ).toEqual([]);
  });

  it("정본에서 사라진 클래스는 표에 「완료」로 적혀 있다", () => {
    const silent = table.filter((r) => !r.done && !canon.has(r.cls)).map((r) => `  .${r.cls}`);
    expect(
      silent,
      "정본에서는 사라졌는데 표가 「완료」가 아니다. 표를 갱신하지 않았다.\n" +
