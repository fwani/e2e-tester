...
| `Tooltip.tsx` | `TooltipProvider` · `Tooltip` · `Truncate` | **radix** `Tooltip` | `tooltip` | 잘린 글자·아이콘 단추의 `title` 만으로 보이던 전체 이름 (비활성 사유의 `title` 은 **유지**) | ✅ T063·T064 — 행 메뉴 `⋮` · 알림 `×` · Step 상세 닫기. `Truncate` 는 US3 T074 에서 쓴다 |
