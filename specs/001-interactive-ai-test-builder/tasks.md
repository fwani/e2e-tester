...
- [ ] T162 손상된 실행 결과 파일을 "결과 없음" 으로 보고하지 않는다 per 헌법 §보안(명시적 오류 처리)·FR-087 (partial) — `ProjectRepository.read_result` 가 `except Exception: return None` 으로 삼켜, 결과 파일이 손상되면 API 가 `TEST_NOT_FOUND` "먼저 실행하세요" 를 돌려준다. 목록 조회에서 깨진 파일 하나가 전체를 막지 않게 하려는 원래 의도는 유지하되(`list_tests` 의 `problems` 와 같은 방식), **단건 조회는 손상 사유를 밝히도록** 경로를 나눈다
