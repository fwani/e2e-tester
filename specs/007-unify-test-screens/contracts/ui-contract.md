...
| 국면 작업 영역 최소 높이 | 42px | `RunnerPaused`·`Takeover` 의 국면 안내 띠 (`flex: 0 0 42px`). **미러 안쪽의 170px 는 표본 대상 앱의 사이드바이며 근거가 아니다** |
...
| # | 값 | 근거 |
|---|---|---|
| B1 | `③-a` 세션 없음 높이 **118px** (만들기·편집) | 46px 강조 버튼 + 상하 24px 여백 + 3px 테두리 |
| B2 | `③-b` 헤더 띠 **36px** | `StepInspector.dc.html` 의 LOCATOR 표 헤더 높이와 같은 값 |
...
| B7 | 방법 카드 **선택 표시** — `#FFF9D6` 채움 + `5px 5px 0` 그림자 | 확정 디자인의 강조 버튼과 같은 문법을 카드에 적용한 것 |
...
|---|---|---|
| CRE 만들기 | `fixed 118` — 아직 열지 않음 | `fill` — 시작 조건 |
...
| EDT 편집 | `fixed 118` — 브라우저 열기 | `fill` — Step 편집면 |
