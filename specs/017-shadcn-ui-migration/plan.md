...
2. **동작이 필요한 곳만 Radix** (R2). 대화상자·확인 대화상자·메뉴·탭·분절 선택·툴팁. 체크박스·라디오·
   선택칸·펼침은 **네이티브** — 정본 모습이 네이티브이고, 동작 이득이 없으며, 번들이 8 kB 준다.
...
**Primary Dependencies**: **신규 2개** — `radix-ui` 1.6.x(Dialog·AlertDialog·DropdownMenu·Tabs·ToggleGroup·
Tooltip·Slot, 통합 패키지 이름 가져오기), `class-variance-authority` 0.7.x. shadcn CLI 는 의존성이 아니다 —
원본을 받아 오는 도구로만, 저장소 밖에서 버전을 고정해 쓴다.
**들이지 않는 것**: `tailwind-merge`, npm `cn`, `lucide-react`, `tw-animate-css`, `sonner`, `@radix-ui/react-*`
개별 패키지, Radix `Select`·`Checkbox`·`RadioGroup`·`Collapsible`.
...
1. **조항이 고정한 것은 열거된 항목이다.** Radix 는 React 의 부품 동작 라이브러리이고 React 를 대체하지
   않는다. 화면은 여전히 React 로 그려진다.
2. **조항의 목적** — MVP 중 기반 기술 교체로 인한 재작업 방지. 이 도입은 frontend 표현·상호작용 계층에만
   닿고 backend·Step DSL·교차 언어 스키마 의무와 무관하다.
3. **실행 의존성이라는 차이.** Tailwind 는 빌드 도구였고 `radix-ui` 는 번들에 실린다. 그 대가는 번들 예산
...
**공급망**: npm 의존성 2개(`radix-ui`·`class-variance-authority`)가 늘어난다. 버전을 고정해
...
|---|---|---|
| Radix 메뉴가 `.click()` 으로 열리지 않는다 | `RowMenuVisible` · `TestListActions` · `EditEntryPoints` | 「메뉴가 열린다」 단언을 지운다 |
...
```text
frontend/
├── package.json                  # radix-ui · class-variance-authority 추가
...
│   │   ├── Dialog.tsx · AlertDialog.tsx · OverlayPane.tsx · DropdownMenu.tsx                    # 신규 · Radix
...
|---|---|---|
| **0. 스파이크** | S1 이름공간 비우기 · S2 G-E `cva` · S3 jsdom + Radix · S4 개발 서버 순회 · S5 `:has()` 변종 | 다섯 다 되는 방법 확인. 실패하면 research 의 대안으로 |
...
|---|---|---|
| 015 FR-007 「컴포넌트 라이브러리를 도입하지 않는다」 | 사용자 요청 (017) | 부품 원본 이식 + 동작 층만 Radix (research R1·R2) |
...
| RK-4 | Radix 가 미러 입력과 다툰다 | 한글 조합이 끊김 · 대화상자 중 대상 앱에 입력 | Step 상세는 비모달(R7) · `MirrorInputWithDialog` 테스트 · quickstart H-2 |
