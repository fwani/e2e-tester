...
005 때 변환 함수 검사(`ScreenUrl.test.ts`, 018 에서 `paths.test.ts`·`Routing.test.tsx` 로
옮겨졌다)는 이 결함을 잡지 못했다 — 화면 상태 ↔ 질의 문자열 **변환 함수**를 보고, 변환은
처음부터 옳았다. 틀린 것은 앱이 아직 화면을 정하지 못한 첫 렌더(`loading`)에서 주소를
지운다는 것이었고, 그것은 **실제로 다시 불러올 때만** 드러난다.

`frontend/tests/RecheckPhase12.test.tsx` 의 T118 은 018 부터 앱 전체를 그려 첫 조회 동안
주소가 그대로인지 본다 — "이 조건이 코드에 남아 있는가" 를 jsdom 에서 보는 계층이다.
여기는 실제 브라우저에서 새로 고침을 본다 — "실제로 그렇게 보이는가" 를 보는 계층이다.
