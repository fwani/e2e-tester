...
- [ ] T108 quickstart §1~§5 를 다시 실행한다 — 타입·테스트·단언 수(2275 이상) · 순회(알림 화면 포함 검출 0) · L2(등록되지 않은 차이 0) · 번들(**JS gzip ≤ 210 kB** — T107 에서 실측 207.74 로 다시 잡았다. 옛 상한 177.41 은 갈래 교체가 늘리지 않는다는 전제에서 나온 값이고 그 전제는 반증됐다) · 정본 무변경 · backend 0. 결과를 quickstart 실행 기록과 test-ledger 에 남기고, **알림 자리를 옛 자리로 되돌리면 순회가 실패하는지**(SC-016) 확인한 뒤 되돌린다
