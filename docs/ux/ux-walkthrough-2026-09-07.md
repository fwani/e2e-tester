...
- 걸은 시나리오 **9건**, 발견 **22건** (상 9 / 중 11 / 하 2), 미검증 **3건**.
- **미러(미리보기)에 화면이 안 나오는 원인을 실측으로 특정했다** — 스크린캐스트는 화면이 바뀔 때만 프레임을 만들고, 그 유일한 초기 프레임이 WebSocket 연결 **이전에** 발행되어 버려진다 (U-24).
...
### U-24. 미리보기(미러)가 대상 화면을 보여주지 못한다 — 첫 프레임이 구독 전에 버려지고, 화면이 변할 때까지 다시 오지 않는다 — 영향도: 상 | 화면: Runner·SessionScreen | 시나리오: 전역

- **하려던 것**: 제품 화면 안의 미리보기 영역으로 대상 브라우저가 무엇을 하는지 본다.
- **기대**: 미러에 대상 화면이 보인다. 최소한 현재 화면 한 장은 보인다.
- **실제**: "미러 프레임을 기다리고 있습니다."만 남는다. 대상 크롬 창은 정상이고 실행도
  정상인데 미리보기 영역만 비어 있다. 앞선 S4 관찰에서도 **20초 내내** 이 문구만 떴다
  (U-02 의 관찰 2).
- **증거**: 실측 3단(아래). 스크린캐스트 자체는 정상이라는 것까지 갈라 두었다.
- **왜 그렇게 보이는가** — 두 사실이 겹친다.
  1. **CDP `Page.startScreencast` 는 화면이 바뀔 때만 프레임을 만든다.** 정적 화면에서는
     시작 직후 초기 한 장뿐이고 그 뒤로는 아무것도 오지 않는다.
  2. **그 초기 한 장이 아무도 듣지 않는 시점에 발행된다.** 미러는
     `POST /api/sessions` 안에서 켜지고(`api/routes/sessions.py:425` `await work.mirror.show(0)`),
     프론트의 WebSocket 은 응답을 받아 화면이 마운트된 뒤에 붙는다. 구독자가 없는 동안
     `SessionEventHub.publish` 는 `if not self._sockets: return` 으로 조용히 버린다
     (`api/ws/session_events.py:59`). 계약이 "재전송하지 않는다"로 못박혀 있어
     (`api/ws/session_events.py:6`) 나중에 붙은 구독자에게 현재 화면을 줄 경로가 없고,
     마지막 프레임을 들고 있는 캐시도 없다.
- **실측** (제품 모듈 그대로, 미러를 먼저 켜고 2초 뒤 구독을 붙였다):
  ...
  스크린캐스트 자체는 정상이다 — 별도 프로브에서 headed·창 가림 상태에서도 10.2 fps 가
  나왔고, `--disable-backgrounding-occluded-windows` 유무에 차이가 없었다(10.2 vs 10.0).
  **창 가림도, ack 실패도, headless 여부도 원인이 아니다.**
- **왜 문제인가**: 미러는 제품 안에서 대상 화면을 볼 수 있는 **유일한** 수단이고(조작 영역이
  아니라는 것이 FR-047a 로 고정되어 있다), 세션이 살아 있다는 것을 눈으로 확인하는 수단이다
  (FR-047d). 그것이 비어 있으면 사용자는 "실행이 멈췄나"와 "화면이 안 변하나"를 구분할 수
  없다. 특히 대상 앱을 막 열었을 때(녹화 시작 직후)와 오래 기다리는 Step 에서 항상 빈다 —
  **정확히 사용자가 화면을 가장 보고 싶은 두 순간이다.**
- **개선 방향**:
  1. **마지막 프레임을 캐시하고 구독이 붙을 때 한 장 보낸다.** `TabScreencast` 가 최근
     프레임을 들고, `SessionEventHub.connect` 가 구독 직후 그것을 밀어 준다. 계약의
     "재전송하지 않는다"는 순서 있는 상태 이벤트를 두고 한 말이고, `mirror_frame` 은 이미
     "유실 가능 · 마지막 프레임만 그리면 된다"로 정의돼 있어(websocket.md:62) 한 장
     재전송은 계약과 어긋나지 않는다.
  2. **무프레임 감시를 둔다.** 마지막 프레임 후 2초 넘게 조용하면 스크린샷 한 장을 같은
     이벤트로 보낸다. 강등 경로(`mirror/screencast.py:_screenshot_loop`)가 이미 같은 일을
     하므로 재사용할 수 있다. 오래 기다리는 Step 에서 미러가 비는 문제가 이것으로 잡힌다.
  3. 빈 상태 문구를 사실에 맞춘다. "미러 프레임을 기다리고 있습니다" 대신 **"대상 화면이
     변하면 여기에 표시됩니다"** — 지금 문구는 곧 올 것처럼 말하지만 정적 화면에서는
     오지 않는다.
- **손대는 범위**: 소~중 (`mirror/screencast.py`, `api/ws/session_events.py`,
  `components/MirrorView.tsx`) · **제약 확인**: FR-047a 저촉 없음(전송 방향만 손댄다).
  FR-047b 저촉 없음(실행 경로와 분리 유지).
