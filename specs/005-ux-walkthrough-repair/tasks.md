...
- [X] T110 [quickstart.md](./quickstart.md) §2 의 S1~S8 을 사람이 직접 걸어 **상 9건 재발 여부**를 확인한다. 하나라도 재발하면 U 번호를 들어 남은 작업으로 등록한다 (SC-221)
- [X] T111 quickstart.md §3 의 기존 성질 8개가 유지되는지 확인한다 — 특히 재실행 경로에 언어모델 없음(원칙 II), 일시정지가 브라우저 상태 유지(원칙 III), 미러가 입력 전달 없음(FR-047a)

---

## Phase 12: T110 재발 검증이 등록한 남은 작업

**출처**: [docs/ux/ux-recheck-005.md](../../docs/ux/ux-recheck-005.md) — quickstart §2 의
S1~S8 을 다시 걸어 「상」 9건을 재점검했다. **해소 7 · 부분 재발 2.**

부분 재발 2건(U-03·U-04)은 **핵심 증상이 사라지고 곁가지 문구·잔여 UI 가 남은** 형태다.
중지는 더 이상 `FAIL` 로 기록되지 않고(`outcome = "stopped"` 실측), 일시정지는 +0.3초에
전이를 말한다. 남은 것은 그 화면들이 **끝난 뒤를 정리하지 않는 것**이다.

quickstart §4 가 "조용히 넘기지 않는다" 고 정한 대로 U 번호를 달아 등록한다.

### 부분 재발 (US2·US3 후속)

- [X] T112 [US2] 재실행 세션 제목의 「초안」을 없앤다 — **U-03-a** (FR-134 미달). 저장 경로(T079)만 고쳐졌고 재실행 세션은 여전히 「TC-001 초안」이다. 같은 화면 부품이 저장 세션에서는 「TC-003 · 저장됨」을 쓴다 **원인**: 판정이 `savedAt !== null`("이 세션에서 저장했는가")이었고 그것을 "저장된 테스트인가" 로 읽었다. 재실행 세션은 저장한 적이 없지만 파일은 처음부터 있다. **수정**: `wording.ts:sessionTitle()` 이 `persisted`(대상 테스트 존재)·`savedAt`·`hasUnsavedChanges` 세 값으로 판정하고, `SessionScreen.tsx` 가 `persisted={testId !== null}` 을 넘긴다. 저장하지 않은 편집이 남으면 「저장하지 않은 변경 있음」이다 — 「저장됨」을 단정하지 않는다
- [X] T113 [US2] 중지 후 화면에서 저장 프롬프트(「테스트 이름」 + 「저장」)를 걷고 ui-contract §8 의 네 번째 다음 행동 「결과 자세히 보기」를 붙인다 — **U-03-a** (FR-133, 현재 3개) **원인**: 「결과 자세히 보기」의 조건이 `isDone` 이었고 중지 후 상태(`review`)는 `TERMINAL_STATES` 에 **일부러** 없다(DR-010 이 편집·저장을 받기 위해 뺐다). 그 결정이 결과 경로를 함께 지웠다. **수정**: 판단 근거를 "세션이 끝났는가" 에서 "볼 결과가 남았는가"(`isDone || isSaveableWithoutBrowser`)로 바꿨다. 저장 프롬프트는 **결말 화면에서만** 걷는다 — 일시정지 편집 화면에서 감추면 ui-contract §9(FR-156: 저장 후에도 「변경 저장」이 비활성으로 남는다)를 어긴다. 이름 없는 녹화를 중지한 결말 화면에는 남긴다(저장이 유일한 보존 수단)
- [X] T114 [US2] 종료된 세션에 대한 `GET /api/sessions/{id}/tabs` 폴링을 멈추고, 그 404 를 오류 배너로 띄우지 않는다. 배너 문구에서 세션 UUID 를 뺀다 — **U-03-b** (FR-135 미달, 실측 404 3건) **원인 둘**: (1) `resync()` 가 상태와 무관하게 `sessions.tabs()` 를 부르고 그 실패를 `setError` 로 올렸다. (2) T046 은 `sessions.py:work_of` 만 고쳤고 `tabs.py` 는 `str(exc)` 를 그대로 `detail` 로 올렸다 — `SessionError` 의 문장에 진단용 UUID 가 들어 있다. **수정**: `TABLESS_STATES`(종료 상태 + `review`)에서는 탭을 묻지 않고, 탭 조회 실패는 오류로 다루지 않는다(마지막으로 알던 목록을 남긴다). `tabs.py` 는 `sessions.py` 와 **같은 문장**(`SESSION_GONE`)을 쓰고 식별자는 `detail.session_id` 로만 싣는다
- [X] T115 [US3] 실행이 끝나면 일시정지 전이 배너(「…아직 실행 중입니다」)를 지운다 — **U-04-a** (FR-146 의 "낡은 전이 안내를 지운다" 미이행) **원인**: 그 문장은 `session.edit_warnings` 에 들어가고 그 목록은 걷히지 않는다. 편집 경고는 남아야 하는 사실이고 전이 안내는 그렇지 않은데, 둘이 같은 자리에 있어 구분할 근거가 없었다. **수정**: `add_edit_warning(..., transient=True)` 로 표시하고 `clear_transient_edit_warnings()` 로 걷는다. 계약(문자열 목록)은 바뀌지 않는다 — "이건 전이 안내였다" 를 서버 안에서만 기억한다. 정상 종료는 러너가, 중지(취소)는 stop 라우트가 걷는다. **러너의 `finally` 에 두지 않는다** — 이것을 처음 거기 뒀을 때 낡은 가짜 세션에서 `AttributeError` 가 나 완료 신호가 올라가지 않고 러너를 기다리는 단위 테스트가 영구히 멈췄다. `finally` 는 신호만 올린다
- [X] T116 [US3] 미러 오버레이 문구를 실제 상태와 맞춘다 — 전이 중·실행 종료 후에도 「일시정지 · 브라우저 세션과 화면 상태를 그대로 유지하고 있습니다」로 단정한다. 배지와 같은 화면에서 다른 말을 한다 — **U-04-b** (FR-142·FR-146) **원인**: `MirrorPhase` 에 값이 넷뿐이어서 전이 중과 실행 종료가 `paused` 로 뭉개졌다. 배지는 이미 「일시정지 중…」·「실행 종료」를 구분했으므로 한 화면이 두 가지를 주장했다. **수정**: `pausing`·`finished` 를 더하고 문구를 `wording.ts:mirrorNotice()`·`mirrorEmptyMessage()` 로 옮겼다(005 T107 · 006 T084 의 규칙). `SessionScreen` 의 국면 판정 순서는 조작 > 종료 > 실행 끝남 > 전이 중 > 일시정지 — "실행이 끝났다" 가 더 최근의 사실이므로 전이보다 먼저 본다
- [X] T117 [US3] 「멈추기 전에 실행이 끝났습니다」 화면에 실패 사유와 「결과 자세히 보기」를 붙인다 — **U-04-c** (FR-146, 요약 한 줄만 있어 진단 경로를 잃는다) **원인**: `finishedWhilePausing.failureReason`·`onShowResult` 는 props 에 있었지만 **그리는 곳이 없었다.** 헤더 부제가 요약만 썼고, 헤더의 「실행 결과 보기」는 `isDone` 조건 뒤에 있어 일시정지 상태에서는 나오지 않았다. **수정**: 중지 결과 화면(§8)과 같은 형태의 결말 블록을 추가했다 — 요약 + 실패 사유 + 「결과 자세히 보기」. 헤더 부제는 제목만 말한다(FR-140: 같은 요약을 한 화면에 두 번 두지 않는다)

### 재점검이 새로 찾은 것 — 005 요구사항에 걸리는 것만

- [X] T118 [US7] 새로고침이 화면 상태를 잃는다 — 결과 화면(`?screen=result&test=…`)에서 F5 하면 목록으로 떨어지고 주소가 `/` 로 초기화된다. **FR-166·FR-167 미충족** (T095·T096 은 뒤로가기 이탈 방지만 이행했다). 뒤로가기는 통과 — **N-01** **이미 해소돼 있었다** — 006 T046 이 `useScreenUrl.ts:99` 에 `if (current.name === "loading") return;` 을 넣어 원인을 제거했다(첫 렌더의 `loading` 화면이 `initialLocation()` 을 읽기 전에 주소를 지웠던 것). **남은 구멍은 테스트였다**: `ScreenUrl.test.ts` 는 변환 함수만 보고 그 국면을 보지 않아, 같은 회귀가 다시 나도 잡지 못한다. `RecheckPhase12.test.tsx` 가 `loading` 의 질의 문자열과 최초 로드 복원을 직접 고정한다
- [X] T119 [US7] 목록 행 배지가 저장된 결말이 아니라 **열린 세션의 존재**를 본다 — 실행이 끝나도 행은 `RUNNING`·「실행 중」에 고착되고, 같은 화면의 배너는 「실패」로 갱신된다. 배너와 행이 다른 말을 한다 — **N-02** (FR-168·FR-169) **원인**: 조건이 `liveSession !== null` 이었다. 실행이 끝나도 세션은 `failed`·`review` 로 등록에 남으므로 영원히 참이다. 005 는 **백엔드에서 같은 착오를 이미 고쳤다** — `active_session_for_test` 가 등록이 아니라 생존을 세게 한 것이 FR-124 였고, 화면 쪽 절반이 남아 있었다. **수정**: `frontend/src/lib/sessionState.ts`(신규)의 `isRunning(state)` 가 유일한 판정 지점이다. 칩과 「실행 중」은 상태를 보고, 복귀 수단(「실행 화면 보기」)은 세션이 열려 있으면 남는다 — **둘은 다른 요구사항이며 하나의 조건으로 처리한 것이 이 결함이었다.** 「결과 보기」도 세션 존재가 아니라 실행 여부로 갈린다(중지 후 목록에서 결과에 도달할 길이 없던 것을 함께 고쳤다)
- [X] T120 [US8] 통과한 실행의 SCREENSHOT 탭이 「불러올 수 없습니다 / 산출물이 지워졌을 수 있습니다」로 말한다. 통과에는 실패 시점 스크린샷이 없는 것이 정상이므로 없다는 사실을 **긍정문**으로 말한다 — **N-03** (FR-173 미달) **원인**: `emptyArtifactMessage("screenshot")` 은 이미 있었지만 **닿지 않았다.** 스크린샷은 `<img src>` 경로를 타고, 실패가 없어도 주소를 요청해 `onError` 가 오류 배너를 띄웠다. **수정**: `failedIndex === null` 이면 요청 자체를 하지 않고 긍정문을 그린다. 요청하지 않으면 오류도 없다
- [X] T121 [US2] 「실패한 Step 건너뛰고 계속」 컨트롤이 화면에 없다 — 백엔드·계약에는 `skip_failed` 가 있으나(`runner.py`·rest-api.md §3-b) 프론트에서 보내는 곳이 0건이라 `partial_pass` 를 UI 로 만들 수 없다. quickstart S3-6 이 확인 불가로 남았다 — **N-04** (FR-136·FR-137 의 화면 절반). **의도적으로 뺀 것인지 먼저 확인한다** **확인 결과 — 의도적 제외가 아니다.** 근거 넷: (1) FR-137 이 "별도 조작으로 분리되어야 하고" 를 요구한다. (2) ui-contract §6-5 가 버튼 이름까지 정해 뒀다(「실패한 Step 건너뛰고 계속」). (3) quickstart §3-6 이 그 버튼을 누르는 절차다. (4) research.md·plan.md 에 제외 결정이 없고, 006 도 손대지 않았다(프론트 `skip_failed` 0건). **작업 분해가 프론트 절반을 만들지 않은 것**이다 — T038 이 백엔드 필드를 세웠고 대응하는 화면 작업이 없다. **수정**: `client.ts:resume(id, skipFailed)`, `RunnerPaused` 에 보조 버튼(강조 없음 — 권하는 길이 아니라 알고 고르는 길), `wording.ts:skipFailureNotice()` 가 누르기 전에 결말이 「부분 성공」이 된다는 사실과 선행 상태 경고를 말한다
- [X] T122 [P] 녹화 일시정지 부제가 없는 Step 번호를 말한다 — Step 5개를 녹화하고 멈추면 「Step 06 이후 정지」다. `RunnerPaused.tsx:274` 가 `stepLabel(currentStepIndex)` 에 **다음 위치**를 넘긴다. `stepLabel` 은 옳고 인자가 틀렸다 — **N-06** (FR-138) **수정**: 한 줄을 고치지 않고 `wording.ts:pausedAfterLabel()` 을 뒀다. 인덱스를 옮기는 판단이 화면에 있으면 같은 실수가 다시 난다 — 그것이 FR-138 이 사전을 만든 이유다. `currentStepIndex - 1` 을 **같은 `stepLabel()` 에** 넘기므로 변환 규칙은 하나로 남는다. 실행 전(`0`)은 「첫 Step 실행 전 정지」다 — 없는 Step 번호를 말하지 않는다

### 005 범위 밖 (다음 기능에서 다룬다)

`docs/ux/ux-recheck-005.md` 의 **N-05**(결말·상태 어휘가 화면마다 한글/영문으로 갈린다 —
러너 「완료」 ↔ 결과 화면 `PASS` ↔ 목록 `PASS`)와 **N-07**(`/api/health` 의 `bind` 가
`--port` 를 반영하지 않는다)은 여기에 등록하지 않는다. N-05 는 ui-contract §1 이 문장과
칩을 **의도적으로** 나눈 결과이므로 어휘 계약 자체를 다시 정하는 일이고, N-07 은 화면에
나오지 않는 진단 값이다. 둘 다 이 기능의 요구사항에 걸리지 않는다.
...
- [X] T123 `frontend/src/pages/SessionScreen.tsx:670` 의 결말 요약 배너를 `finishedWhilePausing` 에서도 걷는다 per FR-140 (contradicts). 조건이 `summary !== null && !isDone` 이고, 멈추기 전에 실행이 끝난 경우는 `paused` 이므로 `isDone` 이 거짓이다 — 배너와 `RunnerPaused` 의 결말 블록이 **같은 요약을 한 화면에 두 번** 그린다. T042 가 얇은 회색 띠를 없앤 것과 같은 결함이 다른 조건으로 남아 있었다(그때는 헤더 부제가 상대였고 지금은 결말 블록이다). `frontend/tests/OutcomeVocabulary.test.tsx` 가 이 조합을 보지 않아 통과했다 — 단정을 그 조합까지 넓힌다 **수정**: 조건을 `!isDone && !finishedWhilePausing` 으로 바꿨다 — 조건의 뜻은 "실행 화면이 요약을 스스로 그리지 않는 경우" 이고, 종료 상태(`Runner` 의 결말 바)와 멈추기 전 종료(`RunnerPaused` 의 결말 블록)가 둘 다 그린다. `OutcomeVocabulary.test.tsx` 가 `SessionScreen.tsx` 원문에서 그 조건을 뽑아 `!isDone` 과 `!finishedWhilePausing` 을 **둘 다** 요구한다(세션 화면을 통째로 그리면 실시간 통로·미러까지 흉내 내야 하고 그러면 무엇을 재는지 흐려진다 — `screen-blockers.test.tsx` 가 같은 이유로 같은 방식을 쓴다). **역검증**: 가드를 지우면 그 단정이 정확히 그 이유로 실패한다
- [ ] T124 **미수행 — 사람이 해야 한다.** [quickstart.md](./quickstart.md) §2 의 S1~S8 을 **사람이 다시 걸어** 「상」 9건이 전부 해소됐는지 확정한다 per SC-221 (missing). T110 은 그 시점의 관측(해소 7 · 부분 재발 2)이고 Phase 12 가 그 2건을 닫았다. **자동 테스트로 대체할 수 없다** — T121 이 그 증거다: 계약·백엔드·자동 테스트가 모두 통과하는데도 화면 절반이 없었고, 그것은 사람이 걸어야 드러났다. 결과는 `docs/ux/ux-recheck-005.md` 형식으로 남기고, 재발이 있으면 U 번호를 들어 등록한다 **검증 준비 완료 (2026-09-07)** — 진행 대본 [docs/ux/recheck-protocol-005.md](../../docs/ux/recheck-protocol-005.md)(판정 기준 3값·Phase 12 가 손댄 자리별 확인 항목·세션 전후 절차), 기록 시트 [docs/ux/recheck-sheet-005.csv](../../docs/ux/recheck-sheet-005.csv)(18행, `verdict` 칸은 **비어 있다**). **판정은 들어 있지 않다** — 비운 칸을 「해소」로 읽지 않는다
- [ ] T125 **미수행 — 사람이 해야 한다.** quickstart §3 의 S3-5·S3-6 을 걸어 **「실패한 Step 건너뛰고 계속」의 결말이 `부분 성공`** 인지 확인한다 per FR-137 · quickstart S3-6 (missing). 재점검의 「미검증」 사유(UI 컨트롤 부재)는 T121 이 없앴으나, 그 결말을 **제품을 통해 관측한 적이 아직 없다.** 백엔드 경로는 `backend/tests/integration/test_resume_with_failed_step.py`(T030)가 지키고 화면은 `frontend/tests/RecheckPhase12.test.tsx` 가 지키지만, 둘을 잇는 한 바퀴는 아무도 돌지 않았다 **검증 준비 완료 (2026-09-07)** — 진행 대본 [docs/ux/recheck-protocol-005.md](../../docs/ux/recheck-protocol-005.md)(판정 기준 3값·Phase 12 가 손댄 자리별 확인 항목·세션 전후 절차), 기록 시트 [docs/ux/recheck-sheet-005.csv](../../docs/ux/recheck-sheet-005.csv)(18행, `verdict` 칸은 **비어 있다**). **판정은 들어 있지 않다** — 비운 칸을 「해소」로 읽지 않는다
- [X] T126 실브라우저에서 **결과 화면 새로고침 복원**을 단정한다 per FR-166 · SC-219 (partial). 자동 커버리지가 jsdom 수준(변환 함수 · `initialLocation()`)뿐이어서 N-01 을 잡지 못했다 — 원인은 첫 렌더의 `loading` 국면이었고 변환은 처음부터 옳았다. 수단은 이미 있다: `backend/tests/abnormal/ui_context.py` 의 실브라우저 하니스(`AS-026` 이 `page.reload()` 를 쓴다). 결과 화면에서 새로고침 후 주소와 화면이 유지되는지 보는 항목을 넣는다 **완료** — `backend/tests/abnormal/test_ui_state_restore.py` 3건. 결과 화면 F5 후 **주소와 화면 둘 다** 유지되는지, 목록에서는 파라미터를 남기지 않는지, 뒤로가기가 앱을 벗어나지 않는지. 결과 화면의 확인은 고유 표지(`SCREENSHOT` 증거 탭)를 **긍정형**으로 본다 — "목록이 아니다" 를 부정으로 재면 두 화면이 공유하는 머리글에 걸려 우연히 통과한다. **역검증**: `useScreenUrl.ts` 의 `loading` 가드를 지우면 「새로고침이 주소를 잃었다」로 실패한다(N-01 재현). 하니스 픽스처(`ui_browser`·`ui_context`)를 `tests/abnormal/conftest.py` 로 옮겼다 — 복사하면 한쪽이 낡고, `_FakeSession` 이 정확히 그렇게 됐다
- [X] T127 [P] `frontend/tests/` 에 `resume` 요청 본문이 실제로 `{"skip_failed": true}` 를 싣는지 단정하는 테스트를 넣는다 per FR-137 (partial). 지금 단정은 콜백이 불렸는지까지다. **N-04 가 드러낸 이음매가 정확히 이 자리다** — 계약(rest-api.md §3-b)과 백엔드는 서 있었고 전선만 없었으며, 그것을 보는 테스트가 없어 아무도 몰랐다. 건너뛰지 않는 재개가 본문을 **보내지 않는** 것도 함께 단정한다(기존 클라이언트 동작 유지) **완료** — `frontend/tests/ResumeRequestBody.test.ts` 4건. 나가는 요청의 본문을 직접 본다: 건너뛰기는 `{"skip_failed": true}` 를 **필드 이름까지** 단정하고(카멜케이스가 나가면 서버는 조용히 `false` 로 읽는다 — 사용자에게는 버튼이 아무 일도 하지 않는 것으로 보인다), 보통의 재개는 본문을 **보내지 않는다**(계약: 본문 없음 = `skip_failed: false`)
- [X] T128 [P] `frontend/src/lib/sessionState.ts` 의 `isResumable()` 을 쓰거나 지운다 per plan: 구조 결정 (unrequested). 「실행 화면 보기」의 조건이 `liveSession !== null` 인데 이 함수는 `stopped` 를 제외한다 — 둘 중 어느 것이 맞는지 정해야 한다. 쓰지 않는 판정 함수를 남기면 다음 사람이 그것이 이미 적용된 규칙이라고 읽는다 **지웠다.** `stopped` 로 가는 유일한 길은 `discard` 이고(`state_machine.py`: `DISCARD → STOPPED`) 그 경로가 세션 등록을 함께 지운다(`sessions.py`: `_WORK.pop`). 그래서 `GET /api/sessions` 의 목록에 `stopped` 는 **나타날 수 없다** — 이 판정은 미사용이 아니라 구조적으로 죽은 코드였다. 목록에 있는 세션은 전부 돌아갈 수 있고 행의 조건 `liveSession !== null` 이 이미 그것을 정확히 말한다. 근거를 `sessionState.ts` 주석으로 남겼다 — 없다는 것이 결정의 기록이다
- [X] T129 [P] `onResumeSkippingFailure` 의 시그니처를 실제와 맞춘다 per FR-137 (partial). 인자로 실패 Step 인덱스를 받지만 `SessionScreen.tsx` 의 구현은 그것을 버린다 — 백엔드가 실패 Step 을 스스로 찾기 때문이다(`resume` 의 `skip_failed`). 인자를 지우거나, 백엔드가 그것을 쓰게 한다. 받고 버리는 인자는 다음 사람에게 "이 인덱스가 반영된다" 고 거짓말한다 **인자를 지웠다.** 백엔드가 건너뛸 Step 을 스스로 찾고(`sessions.py:resume` 의 `first_failed_index()`) `skip_failed` 는 순수한 불리언이므로 인덱스를 반영할 곳이 없다. 이 과정에서 `onClick={fn}` 이 React 이벤트를 첫 인자로 넘기는 것을 함께 잡았다 — 인자를 받지 않는다는 계약이 호출 자리에서 깨져 있었다. 테스트가 호출 인자 수가 0 인지 단정한다
