...
**Scale/Scope**: 1회차 7국면 · 33개 조작 · 삭제 대상 4개 페이지 파일 + 3개 페이지의 껍데기.
**2회차 8국면 · 34개 조작** (`record.start` 추가) · 삭제 대상 2개 페이지 파일
(`CreateTest`·`AiCompose`) · 현재 관련 코드 약 5,000줄
...
## Phase 2b — 2회차 이행 순서 (2026-09-08)

`research.md` R13 의 결정. 1회차의 8단위가 끝난 뒤에 온다. **두 묶음이며 순서를 바꾸지
않는다.**
...
**묶음을 더 쪼갤 수 없는 이유** (1회차 R7 과 같은 형태): A 를 쪼개면 어떤 국면은 새 규칙,
어떤 국면은 옛 규칙이 되어 FR-256 이 성립하지 않는다. B 를 쪼개면 껍데기가 3벌인 상태가
유지된다.

### 2회차 Constitution 재확인

판정은 바뀌지 않는다. 설계가 원칙을 더 강하게 만든 지점 둘:

| 원칙 | 2회차가 더한 것 |
|---|---|
| **II. Deterministic Replay** | 세로 배분이 `Record<Phase, VerticalSplit>` 정적 표다. 크기 판정에 조건 분기가 없고 언어모델이 닿는 자리도 없다. `PHASE_TABLE` 과 같은 규율 |
| **III. Stateful Interactive Runner** | 만들기 국면이 흡수되면서 세션 생성 경로가 **어댑터 하나**로 모인다. 지금은 `CreateTest`·`AiCompose` 둘이 각각 `sessions.create` 를 부른다 — 경로가 줄어드는 방향이다 |

게이트 4(테스트 비활성화 금지)가 2회차에도 가장 큰 위험이다. 개명(`PhaseAside` → `WorkArea`)
이 선택자를 바꾸고, 만들기 흡수가 `CreateTest`·`AiCompose` 테스트를 통째로 옮기게 한다.
**갱신으로만 통과시킨다.**
...
| 항목 | 왜 지금 완결되지 않는가 | 지금의 완화 | 회복 경로 |
|---|---|---|---|
| **`CreateTest.dc.html` 은 확정 디자인이다** | 2회차 artboard 묶음은 초안이고, 초안이 확정을 대체할 수 없다 (FR-254c · research R14) | 승인 전까지 만들기 국면은 **`CreateTest.dc.html` 의 요소를 잃지 않았는지**로 대조한다 — 시작 URL 필드 · 방법 2택의 설명 문구 · 취소. 자리 이동은 대체 관계로 기록하고, 요소 소실은 DC-007 위반 | `docs/PENDING-HUMAN-VERIFICATION.md` 에 등록. 승인 후 B1~B10 이 바뀌면 그 범위만 고친다 |
| **설계 초안이 범위를 넘었던 것** | `Create.dc.html` 초안이 「테스트 이름」과 「빈 테스트」를 그렸다. 둘 다 제품에 없는 조작이다 | Phase 0 대조에서 걸러 초안과 FR-258 을 함께 고쳤다 (FR-258a · research R11). **초안을 근거로 구현하면 새 조작이 들어간다** | 이름을 만들기 국면에서 정하는 것은 별도 기능으로 제기해야 한다. 지금은 저장 시점(일시정지 국면)이 유일한 자리다 |
...
- **만들기 국면의 테스트 이름 입력** (2회차) — 새 조작이므로 범위 밖이다 (FR-258a).
  이름은 저장 시점에 정한다
- **`fixed` 배분의 창 높이 대응** (2회차) — `fixed 424`(결과)·`fixed 118`(편집·만들기)은
  기준 높이 900 을 전제한다. 그보다 낮은 창에서 어떻게 되는지는 기존 좁은 창 정책
  (재배치하지 않고 스크롤 · DC-011)을 따르며, 세로 정책을 새로 만들지 않는다
