...
### 3-3-0. 「자리」는 팔레트가 갖고 행은 **그 조작의 사례**다 — 구현 중 확인한 사실

**초안은 `ActionPalette` 의 `hidden` 으로 자리를 행에 양도하려 했다. 그것은 틀렸다.**

이 저장소에는 007 이 세운 규칙이 둘 있고 검사가 그것을 지킨다.

| 규칙 | 검사 | 내용 |
|---|---|---|
| 감춰진 조작 0건 | `CapabilityUI.test.tsx` | 표에서 `–` 가 아닌 칸은 화면에 `[data-action="<id>"]` 로 **있어야** 한다 |
| 한 조작에 한 자리 | 같은 파일의 `duplicated()` | 같은 `data-action` 이 **둘 이상** 있으면 실패한다 |

행에 `data-action` 을 달면 20~50 행에서 같은 id 가 그만큼 생겨 둘째 규칙이 깨진다.
팔레트에서 숨기고 행에만 달면 첫째 규칙과 충돌한다.

**선례가 이미 답을 갖고 있다.** `step.select` 는 모든 행이 할 수 있는 조작인데
`data-action="step.select"` 는 **Step 패널 하나**에 있다 (`StepList.tsx`). 자리를 선언하는
것은 **담는 것**이고, 행은 그 조작의 **사례**다 — 그래서 행에는 표식이 없다.

이동·삭제·추가도 같다.

| 무엇 | 어디 | 표식 |
|---|---|---|
| 조작의 **자리** (표가 요구하는 것) | 팔레트 | `data-action="step.moveUp"` … |
| 행에서의 **사례** (왕복을 없애는 것) | Step 행 칸 5 | `data-row-action="step.moveUp"` … |

**팔레트에서 빼지 않는다.** 표가 그 국면에 그 조작을 두라고 하면 선언된 자리가 있어야
하고, 그 자리가 팔레트다. 행 조작은 그 자리를 대신하지 않고 **짧은 길**을 더한다 —
FR-298 이 요구하는 것은 「행에서 할 수 있다」이고 「팔레트에서 없앤다」가 아니다.

팔레트 경로도 동작해야 한다. 그것이 곧 표가 거짓말하지 않는다는 뜻이다 — 화면에 보이는
조작을 눌렀는데 아무 일이 없으면 그것이 005 U-06 의 형태다.

### 3-3-1. 읽기 전용 국면에는 칸 5 를 그리지 않는다

**결과 국면(RES)에서는 행에 칸 5 가 없다.** 조작은 팔레트에 비활성으로 남는다 — §3-3-0
아래에서는 그것이 원래 선언된 자리이므로 「옮기는」 것이 아니라 **사례를 더하지 않는** 것이다.

**근거.** 그 국면의 이동·삭제·추가는 `○ RESULT_NO_EDIT → nav.editStep` 이다. 행이 사례를
가지면 **행마다 같은 이유의 비활성 조작 4개**가 생기고 20~50 Step 목록에서 최대 200개가
된다. 결말을 읽는 화면이 쓸 수 없는 조작으로 덮인다.

FR-234 를 어기지 않는다 — 조작은 팔레트에 **있고** 이유와 갈 길을 갖는다.

| 국면 | 칸 5 (사례) | 팔레트 (선언된 자리) |
|---|---|---|
| REC · AI · TKO · RUN · PAU · EDT | **있다** | 있다 |
| CRE 만들기 · RES 결과 | 없다 | 있다 (비활성 + 이유) |

**구현**: `Workbench` 의 `rowActions` 를 넘기지 않으면 칸 5 자체가 그려지지 않는다
(`StepList.tsx` 의 `actions !== undefined` 분기). 결과 화면과 만들기 화면은 넘기지 않는다.
...
**거절** — 오류 코드는 전부 `DEFINITION_INVALID` 다 (003 EC-006 이 코드를 늘리지 않기로
했다). **상태 코드가 갈리는 것은 누가 거절했는지의 차이다.**

| 상황 | 상태 | 누가 | 문구 |
|---|---|---|---|
| `at` 이 범위를 벗어남 | **400** | 라우트(`_apply_edits`) | 「넣을 위치가 범위를 벗어났습니다: 99 (Step 5개)」 |
| `kind` 가 목록에 없음 | **422** | 전역 검증 핸들러 | 요청의 *모양*이 계약에 맞지 않는다. 필드 위치만 싣는다 |
| `target` 을 실어 보냄 | **422** | 전역 검증 핸들러 | `extra="forbid"` 가 막는다 |
| 필수 필드 누락 | **422** | 전역 검증 핸들러 | 같음 |
| `revision` 누락 | **422** | 전역 검증 핸들러 | 지금과 같다 |
| `revision` 불일치 | **409** | 라우트 | 지금과 같다 (006 FR-209) |
