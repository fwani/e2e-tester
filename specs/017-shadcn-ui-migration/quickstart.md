...
### 2026-09-16 · Phase 10 마감 — quickstart §1~§5 전량 재실행 (T108)

| § | 확인 | 결과 |
|---|---|---|
| 1-1 | 타입 검사 · 테스트 · 단언 수 | `tsc` 통과 · **1419 / 1419 통과** (파일 118 · 연속 2회) · 단언 **2279** (기준 2275) · 무른 단언 **639**(기준선과 같다) · 건너뜀 0 |
| 1-4 | 정본 무변경 | `git diff 75520ee -- src/theme/tokens.css` **0줄** |
| 2 | 화면 깨짐 순회 | **19화면 × 4폭 = 76회** · **등록되지 않은 검출 0** · 닿지 못함 0 · 허용 28(사유 있음) |
| 3 | L2 대조 | 8화면 × 56속성 · **대조 34272칸** · 불일치 0 · 의도된 차이 188 |
| 4 | 번들 | CSS gzip **6.34 kB** (≤ 6.78) · JS gzip **207.74 kB** (≤ **210** — T107 재개정) · 금지 의존성 없음. 런타임 의존성은 넷뿐이다: `@base-ui/react` · `class-variance-authority` · `react` · `react-dom` |
| 5 | 원칙 II | `git diff 75520ee -- backend/` **0줄** |
| SC-016 | **가드가 실제로 잡는가** | ✅ 아래 |
| H | 사람 판정 | H-2 · H-6 · H-8 · **H-9** · **H-10** 미실행 — `docs/PENDING-HUMAN-VERIFICATION.md` |

**SC-016 — 순회가 「초록이라서」가 아니라 「잡을 수 있어서」 초록임을 확인했다.** 알림 자리를 옛 자리
(오른쪽 위)로 되돌리고 순회를 돌렸다:

```
sweep exit 1 · 등록되지 않은 검출 4
✗ [edit-notice@1280] covered button{전부 고르기} by div{먼저 저장해야 합니다…}
✗ [edit-notice@1440] · [edit-notice@1920] · [edit-notice@2560] — 같은 것
```

네 폭 모두에서 **N-02 가 그대로 되살아났다.** 자리를 되돌리자 `sweep exit 0 · 검출 0` 으로 돌아왔고
`git diff` 가 비어 파일이 원래대로임을 확인했다.

**한 번 보였다가 재현되지 않은 것 하나**: 전량 1회차에서 `DeleteOutcome` 의 「섞여 있으면 둘 다 말한다」가
`expected null not to be null` 로 실패했다. 그 파일 단독 **3회**, 전량 **2회** 모두 통과해 재현되지
않았다 — **파일 사이 순서에 기대는 일회성**으로 본다. 「불안정하다」고 단정하기에는 관찰이 한 번뿐이라
**고치지 않고 사실만 남긴다**; 다시 보이면 그때 원인을 찾는다.
