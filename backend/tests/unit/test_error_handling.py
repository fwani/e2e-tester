...
EXPECTED_BARE_SUPPRESS: dict[str, int] = {
    ...
    # 10번째 (010 T020): 상태 전이를 조작 채널에 알리는 통보 경로다 (FR-342). **채널의
    # 사정이 상태 전이를 실패시켜서는 안 된다** — 조작 채널의 어떤 일도 실행 상태 기계를
    # 건드리지 않는다는 것이 계약의 불변식 4 이고 (contracts/mirror-control.md §5), 여기서
    # 예외를 흘리면 채널 하나 때문에 일시정지가 실패한다. 사유를 남길 자리도 없다 —
    # 통보를 받는 쪽이 이미 자기 사정을 안다.
    "session.py": 10,
    "artifacts.py": 6,
