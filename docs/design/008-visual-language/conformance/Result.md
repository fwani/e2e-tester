...
> 이 아트보드는 통합 작업 화면의 **「결과」 국면** 하나다. 껍데기는 다른 국면과 같아야 하고(FR-217) 담는 것만 다르다.
...
## L3 — 구조·가감 (사람이 채운다)
> **2026-09-08 (009) — 이 국면의 행에는 칸 5 를 그리지 않는다.**
>
> `.srow` 격자는 18장 공통이라 다섯째 칸이 생겼지만, **읽기 전용 국면에서는 그 칸을
> 채우지 않는다** (`specs/009-step-editing-flow/contracts/step-editing.md` §3-3-1).
>
> 근거: 이 국면의 이동·삭제·추가는 `○ RESULT_NO_EDIT → nav.editStep` 이다. 행마다 같은
> 이유의 비활성 조작 4개를 20~50행에 반복하면 결말을 읽는 화면이 쓸 수 없는 조작으로
> 덮인다. 조작은 팔레트에 이유와 갈 길을 갖고 남는다 — 감춘 것이 아니다 (FR-234).
...
```bash
open docs/design/008-visual-language/Result.dc.html    # 확정 디자인 (그대로 열린다)
