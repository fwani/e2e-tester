...
def _headless_browsers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    **010 이후 이 픽스처는 제품 기본값과 같은 값을 세운다** (FR-352). 그전에는 제품
    기본값이 「창을 띄우는 것」이었고 이 픽스처가 그것을 뒤집는 장치였다. 지금은 창 없음이
    양쪽의 기본값이므로, 픽스처가 없어도 대부분의 검증은 같게 돈다.

    **그래도 지운다는 선택은 하지 않는다.** 이 픽스처가 지금 하는 일은 둘이다.

    1. **개발자의 셸 환경을 덮는다.** 누군가 `ITB_HEADLESS=0` 을 켜 둔 채 전량 실행을
       돌리면 브라우저 세션 60여 개가 창을 띄워 화면을 빼앗는다. 초점이 옮겨 가면
       `blur` 에 기대는 녹화 검증이 엉뚱한 사유로 실패한다. 그 실패는 원인이 환경
       변수라는 것을 드러내지 않는다.
    2. **제품 기본값의 변화로부터 검증을 떼어 놓는다.** 검증이 무엇을 전제하는지가 제품
       기본값과 함께 조용히 움직이면, 기본값을 다시 뒤집는 날 원인을 찾기 어려워진다.
       여기서 명시하는 편이 싸다.

    **창을 띄우는 검증은 이 저장소에 없다.** 있다면 그 검증만 `monkeypatch.setenv(
    HEADLESS_ENV, "0")` 으로 명시적으로 반대로 두어야 한다 — 픽스처를 지우는 것이 아니라.

    창 없이 돌아도 검증하는 것은 같다. 스크린캐스트·recorder.js 주입·hover·끌어다
    놓기 모두 headless 에서 동작한다. 010 이후로는 미러 조작(CDP `Input`)도 그렇다
    (research R1·R2).
    """
