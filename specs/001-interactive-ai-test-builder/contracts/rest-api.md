...
봉인은 공개키만으로 되지만 **재실행·AI 작성은 비밀키를 연다.** 암호구로 잠근 키를 여는 경로는 둘이며,
**일상 경로는 화면이다.**

| 경로 | 조작 | 쓰이는 곳 |
|---|---|---|
| 화면 | `POST /api/keys/unlock` `{ "passphrase": "…" }` | 사람이 조작하는 실행 |
| 환경 변수 | 백엔드 프로세스에 `ITB_KEY_PASSPHRASE` | CI·헤드리스 |

`POST /api/keys/generate` 와 `POST /api/keys/regenerate` 에 암호구를 실어 보내면 **그 자리에서 잠금이
풀린 상태로 남는다.** 화면에서 암호구를 받아 키를 만든 사용자가 같은 값을 셸에 다시 넣고 백엔드를
재기동해야 하는 것은 결함이었다 (UX U-26).

암호구는 **확인 후 백엔드 프로세스 메모리에만** 보관한다. 디스크에 쓰지 않고 어떤 응답에도 담지
않는다. 프로세스가 끝나면 사라지므로 재기동 후에는 다시 해제한다. `DELETE /api/keys/unlock` 은 들고
있던 암호구를 버린다 — 되돌릴 수 없는 조작이 아니므로 확인 문구를 받지 않고, 이미 잠겨 있어도
성공한다.

키 상태의 `unlocked` 가 그 결과를 알린다. 암호구가 걸려 있지 않은 키는 항상 `true` 다. `false` 면
민감 변수를 쓰는 실행이 실패하므로, 화면은 실행이 실패하기를 기다리지 않고 미리 해제를 안내한다.

사유를 코드로 가른다 — 조치가 다르다.

| 상황 | 응답 |
|---|---|
| 암호구가 틀렸다 | `400 PASSPHRASE_INVALID` |
| 열 키가 없다 | `404 KEY_MISSING` |
| 암호구가 8자 미만 | `422` (생성 제약과 같은 값) |

잠긴 채로 실행하면 세션은 시작되지만 민감 변수를 실제로 요구하는 Step 이 **"잠겨 있다"** 는 사유와
함께 실패한다 (FR-089f). "키가 없다" 와 구분해서 보고한다.
