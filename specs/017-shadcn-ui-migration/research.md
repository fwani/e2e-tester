...
## R2 — 동작 층: 무엇을 Radix 로, 무엇을 네이티브로

**결정**:

| 부품 | 동작 층 | 이유 |
|---|---|---|
| 대화상자 · 확인 대화상자 | `radix-ui` `Dialog` · `AlertDialog` | 초점 이동·가두기·Esc·되돌리기가 **지금 하나도 없다** (FR-011). 가장 큰 접근성 이득 |
| 행 메뉴 | `radix-ui` `DropdownMenu` | 키보드 열기·화살표·Esc·바깥 클릭이 없다 (FR-012). 지금은 스크롤 1px 에 닫히는 수제 위치 계산이다 |
| 산출물 탭 | `radix-ui` `Tabs` | `aria-pressed` 버튼 줄에 `tablist` 가 없다 |
| 분절 선택 (실행 속도 · 목록 거르기·정렬 · 삽입 종류) | `radix-ui` `ToggleGroup` (`type="single"`) | 서로 배타적인 선택이며 화살표 이동이 없다 (FR-013) |
| 잘린 글자·아이콘 단추의 전체 이름 | `radix-ui` `Tooltip` | `title` 은 키보드로 볼 수 없다 (FR-019). 초점에도 뜬다 |
| 버튼 `asChild` | `radix-ui` `Slot` | 링크를 버튼 모양으로 그리는 자리 |
| **체크박스 · 라디오** | **네이티브 `<input>`** | 아래 |
| **선택칸** | **네이티브 `<select>`** (shadcn `native-select`) | 아래 |
| **펼침(▸/▾)** | **네이티브 `<details>`** | 아래 |
| 입력 · 여러 줄 입력 · 라벨 · 표 · 칩 | 원시 요소 (shadcn 도 Radix 를 쓰지 않는다) | — |
| 토스트 | **기존 `useToastDismiss` 유지** | 아래 R6 |
| Step 상세 판 | `radix-ui` `Dialog` **`modal={false}`**, 포털 없음 | 아래 R7 |
...
**결정**: `StepDetail` 은 `radix-ui` `Dialog` 를 `modal={false}` 로, **포털 없이** 쓴다. 열리면 초점이 판
