...
**서버는 거절하지 않는다. 화면이 먼저 저장한다.** 서버는 미저장 편집의 존재를 알지
못하기 때문이다 — 그것은 화면이 들고 있는 것이고(`hasPendingEdits`, 권한표 조건 C9),
서버에 알리려면 편집을 보내야 하는데 그러면 그것은 이미 저장이다.

> **구현에서 고쳤다.** 이 절의 초안은 확인 대화(저장하고 시작 / 저장하지 않고 시작 /
> 취소)를 새로 만들라고 적었다. 그런데 이 저장소에는 **이미 규칙이 있었다**:
>
> - **006 FR-203** — 「저장하지 않은 변경이 있으면 **먼저 저장한 뒤** 세션을 연다.
>   두 경로가 같은 Step 을 다르게 들고 있는 상태를 만들지 않는다」
> - **011** — `openBrowser` 주석: 「기존 「저장하고 열기」가 그대로 걸린다 — **새 확인을
>   만들지 않는다**」
>
> 재녹화도 세션을 여는 조작이므로 같은 규칙을 받는다. 같은 상황에서 조작마다 다르게
> 굴면 사용자는 어느 쪽이 편집을 지키는지 외워야 한다.
>
> 구현은 `EditView` 의 `afterSaving` 이다 — `openBrowser` 안에 있던 것을 016 이 꺼내
> 둘이 공유한다. **저장이 실패하면 세션을 열지 않는다** (011 FR-374c).
...
**이것은 `201` 뒤에 일어난다.** 실패표에 넣을 수 없다 — 러너는 요청 수명과 분리돼
돌기 때문이다 (research R1). 요청 안에서 앞 구간이 끝나기를 기다리면 Step 20개짜리
앞 구간에서 HTTP 요청이 수십 초 매달린다.

> **초안은 「`409` + 실패한 Step 정보」로 적었고 틀렸다** (수렴 T072). 그 응답을
> 만들려면 러너를 요청 안에서 기다려야 하고, 그것은 이 제품이 재실행에서 세운 규칙을
> 깬다. 계약 쪽을 고쳤다.

실제 흐름:

```
201 (세션 생성)  →  러너가 앞 구간 실행  →  실패
                                            ├ 상태: failed
                                            ├ step_results 에 실패한 Step 과 사유
                                            └ **트랜잭션을 닫는다** ← FR-020
                                               rerecord_changed(null)
                                               rerecord_realign_failed
                                                 (definition_reverted: false)
```

**트랜잭션을 닫는 것이 FR-020 의 「재녹화를 시작하지 않는다」다.** 닫지 않으면 화면은
재녹화 띠를 그리는데 확정도 버리기도 할 수 없다 — 둘 다 `paused` 를 요구하고 세션은
`failed` 다.

판정 근거는 「아직 아무것도 만들지 않았다」이다. 만든 것이 있으면 그 실패는 도착점
구간이 아니라 사용자가 대화로 만든 뒤의 다른 사정이다.

**세션은 남는다** — 어디서 왜 실패했는지 보려면 세션이 필요하다.
...
| `blocked` | `BlockedView \| null` | AI 가 막혀 있으면 그 내용. `ai_blocked` 상태에서만 실린다 |

`BlockedView` (2026-09-11 사용자 보고):

| 필드 | 형 | 설명 |
|---|---|---|
| `attempted` | `str \| null` | 막힌 시점에 시도하던 동작 (FR-070) |
| `reason` | `str` | 왜 못 했는가 |
| `question` | `str \| null` | 사람에게 물을 한 문장. 없으면 사람이 먼저 말한다 |
| `choices` | `list[str]` | 고를 수 있는 것. **서버가 준다** |

**왜 뷰에 싣는가.** 이 셋은 `ai_blocked` 이벤트로만 갔고, 이벤트는 그 순간 붙어 있던
화면에게만 간다. 목록으로 나갔다 돌아온 화면은 상태가 `ai_blocked` 인 것만 알아 「고를
선택지가 없습니다」를 그렸고, 대화 패널은 「위의 답변 칸에 알려 주세요」라고 말하는데 그
칸이 없었다 — 남은 길이 세션을 버리는 것뿐이었다. 005 U-18 이 같은 형태였고 그 고침이
`step_results` 였다.
