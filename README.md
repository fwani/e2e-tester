...
**화면의 조작 요소가 shadcn/ui 부품 층을 지난다 — 모습은 008 정본 그대로, 동작은 표준으로.**

015 까지 화면은 원시 `<button>`·`<input>`·`<select>` 에 Tailwind 유틸리티를 화면마다 입혔고, 대화상자·메뉴·탭은
손으로 만든 판이었다. 보이는 것은 정본과 같았지만 **초점이 대화상자 밖으로 새고, 메뉴는 키보드로 열 수 없고,
탭은 「눌린 단추 넷」으로 들렸다.** 그리고 실제 창에서 재 보니 알림이 국면 띠의 조작을 덮고, 선택칸이 남은 폭을
전부 가져가고, 좁은 창에서 머리띠가 창 밖으로 밀려났다.

- **부품 층** — `src/ui/` 의 부품이 shadcn new-york-v4 의 구조(`cva` 변종 · `data-slot` · `asChild`)와
  `radix-ui` 동작(대화상자 · 확인 대화상자 · 메뉴 · 탭 · 고르기 묶음 · 툴팁)을 갖고, 클래스는 정본 토큰으로 옮겼다.
  화면 밖 원시 조작 요소는 0(미러의 한글 조합 칸 하나만 사유와 함께 예외)이다.
- **알림 자리** — 알림 층이 문서에 있는 띠(머리띠 · 국면 띠)를 읽어 그 아래에서 시작한다. 모달이 열린 동안에도
  보이고 낭독되며, 알림을 닫는 조작이 대화상자를 닫지 않는다.
- **화면 정책** — 데이터 화면(목록 · 가져오기 · 작업 화면)은 창 폭을 채우고, 좁은 창에서는 본문만 가로로
  스크롤하며 머리띠는 창 폭에 선다. 폼 화면(프로젝트 · 비밀 값 · 키 관리)은 읽기 폭으로 가운데 선다.

**들이지 않은 것**: shadcn 의 기본 모습(모서리 · 그림자 · 링 · 흐린 비활성 · 움직임 · 다크 모드) · `lucide-react` ·
`tailwind-merge` · Radix 의 `Select`·`Checkbox`·`RadioGroup`·`Collapsible`(네이티브 요소가 이미 같은 동작을 갖는다).
가드 G-F 가 부품에서 이것들을 막는다.

**검증은 셋이 함께 본다** — 부품의 모습·원시 요소·클래스 실재를 막는 가드(G-B · G-E · G-F · G-G), 전환 전후 계산
스타일을 요소마다 대조하는 L2(`scripts/design_compare_ba.py` · 의도된 차이는 사유와 함께 등록), 실제 서버·실제
재실행 결과로 18화면 × 4폭을 열어 덮임·넘침·잘림·줄바꿈·창 밖 머리띠를 재는 순회(`scripts/screen_sweep.py`).
L2 는 「전과 같은가」를, 순회는 「그 자체로 깨지지 않았는가」를 묻는다 — 전환 전부터 깨져 있던 것은 L2 만으로는
통과한다.

**남은 것**: 키보드 순회 · 한글 입력과 대화상자 · 넓은 창 · 정본 모습 대조 같은 사람 판정과, 전환 중 모습이 달라진
판단 여섯의 확인 (`docs/PENDING-HUMAN-VERIFICATION.md` §17).
