 ...
 * 조작 식별자로 버튼을 집는다.
 *
 * 라벨로만 찾으면 **해소 방법 링크**와 부딪힌다 — 비활성 조작의 이유 옆에 붙는 그
 * 링크는 가리키는 조작과 같은 라벨을 쓴다 (그것이 006 E-03 의 수정이었다). 주 조작을
 * 재는 자리에서는 식별자로 집는 것이 정확하다.
 */
const act = (id: string) =>
...
describe("일시정지 전이 (FR-142~FR-145 · U-04)", () => {
  it("전이 중에는 미러 배지가 「일시정지 중…」이다 — 아직 정지가 아니다", () => {
  ...
  it("전이 중에도 「중지」는 누를 수 있다 (FR-144)", () => {
    ...
    render(<SessionWorkbench {...paused({ pausing: true, busy: true })} />);
    expect(act("run.stop").disabled).toBe(false);
  });
  ...
  it("멈추기 전에 끝난 실행이 실패 사유와 결과 경로를 준다 (FR-146 · U-04-c)", () => {
    ...
    render(
      <SessionWorkbench
        {...paused({
          summary: "실패 · Step 06 에서 실패 · 5 / 7 통과",
          failure: { index: 5, message: "요소를 찾을 수 없습니다: #submit" },
          onShowResult,
        })}
      />,
    );
  ...
  it("실패 사유가 없으면 그 자리를 비워 둔다 — 없는 사유를 지어내지 않는다", () => {
    render(
      <SessionWorkbench {...paused({ summary: "통과 · 7 / 7 통과", failure: null })} />,
    );
...
describe("실패 Step 재개 차단 (FR-136 · U-05)", () => {
  it("실패한 Step 이 있으면 「계속하기」가 잠기고 이유가 보인다", () => {
    render(
      <SessionWorkbench
        {...paused({
          outcomeOf: (_s: Step, index: number) => (index === 5 ? "fail" : "pass"),
        })}
      />,
    );
    expect(act("run.resume").disabled).toBe(true);
  ...
  it("실패가 없으면 잠기지 않는다", () => {
    render(<SessionWorkbench {...paused({ outcomeOf: () => "pass" })} />);
...
describe("종료 후 컨트롤 (FR-147 · U-08)", () => {
  function running(overrides: Record<string, unknown> = {}) {
    return sessionProps({
      view: sessionView({ state: "replaying", test_id: "TC-003", steps: steps(5) }),
      outcomeOf: () => "pass",
      durationOf: () => 90,
      ...overrides,
    });
  ...
  it("실행 중에는 「중지」다", () => {
    render(<SessionWorkbench {...running()} />);
  ...
  it("끝난 실행에서는 「닫기」이고 강조가 없다", () => {
    render(
      <SessionWorkbench
        {...running({ view: sessionView({ state: "completed", test_id: "TC-003", steps: steps(5) }) })}
      />,
    );
    const close = act("run.stop");
  ...
  it("중지 요청 중에는 「중지 중…」으로 잠긴다 (FR-147)", () => {
    render(<SessionWorkbench {...running({ stopRequested: true })} />);
    expect(act("run.stop").textContent).toBe("중지 중…");
    expect(act("run.stop").disabled).toBe(true);
  });

  it("중지 요청 콜백은 한 번만 불린다", () => {
    const onStop = vi.fn();
    render(<SessionWorkbench {...running({ stopRequested: true, onStop })} />);
    const stop = act("run.stop");
    stop.click();
    stop.click();
