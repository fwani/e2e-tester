...
- [X] T070 [US3] (`Artboard` 가 `header` 인자를 받아 머리띠를 가로 스크롤 영역 밖에 두고, 본문은 `min-width 1440 · width 100%`. `grow` 인자는 지웠다 — 데이터 화면은 전부 창을 채운다. 더한 두 겹은 `data-slot` 으로 표시해 L2 가 경로에서 건너뛴다 · 틀 검사 세 파일은 판정을 바꾸지 않고 통과) 화면 정책 — `frontend/src/components/design/Chrome.tsx` `Artboard` 가 `policy: "data" | "form"` 과 머리띠 자리를 받는다. **머리띠는 가로 스크롤 영역 밖**, `data` 본문은 `min-width 1440 · width 100%`, `form` 은 읽기 폭 가운데 (layout-contract-v3 L3). `WorkbenchShell.test.tsx`·`WorkbenchHeight.test.tsx`·`DesignTokens.test.tsx` 의 틀 구조 판정을 갱신하고 test-ledger 에 적는다 (B-11)
- [X] T071 [US3] (데이터 정책: 목록 · 가져오기(1000 → 1440) · 작업 화면. **폼 화면 셋은 `Artboard` 로 옮기지 않았다** — 이미 읽기 폭 `main` 으로 가운데 서며 옮기면 요소 구조만 바뀐다 · 목록 격자 `GRID` 의 늘어나는 칸이 이름 칸(`1fr`)임을 확인 · `ImplementationCount` 의 `Artboard` 규칙은 그대로 의미가 있다) 화면마다 정책을 배정한다 — `frontend/src/pages/TestList.tsx`·`ImportPreview.tsx`·`frontend/src/components/workbench/Workbench.tsx` → `data`, `frontend/src/pages/ProjectSetup.tsx`(960)·`SecretValues.tsx`·`KeyManagement.tsx`(720) → `form`. 목록 격자(`TestList.tsx` `GRID`)의 늘어나는 칸이 이름 칸인지 확인한다. `ImplementationCount` 의 `Artboard` 사용 규칙이 여전히 의미를 갖는지 확인 (B-10 · T070 의존)
- [X] T072 [US3] (`STEP_FOOTER_MAX_CLASS: Record<Phase, string>` — 편집·만들기 `calc(100%-36px-8*var(--h-step))`, 그 밖 52%. `Workbench` 가 국면으로 조회해 `StepList` 의 `footerMax` 로 내린다) Step 패널 세로 배분 — `frontend/src/lib/layout.ts` 에 국면별 바닥 상한 표(`Record<Phase, …>`)를 두고 편집·만들기에 `calc(100% - 36px - 8 * var(--h-step))` 를 배정한다. `frontend/src/components/workbench/StepList.tsx:470` 의 `max-h-[52%]` 리터럴을 표에서 내려받게 바꾼다 (layout-contract-v3 L4 · B-03). `VerticalSplit`·`StepRowLayout`·`SavePlacement`·`RerecordBandPlacement` 통과, 판정 방법이 바뀌면 test-ledger
- [X] T073 [US3] (`ComposePhase` 의 단언 둘만 바꿨다 · `TargetPane`·`ComposeView` 의 낡은 118px 주석을 고쳤다) 만들기 국면 대상 앱 자리 — `frontend/src/lib/layout.ts` `VERTICAL_SPLIT.composing.targetSlot` 을 `content` 로. `frontend/src/components/workbench/TargetPane.tsx`·`frontend/src/pages/ComposeView.tsx` 의 낡은 높이 주석(118)을 고친다. `VerticalSplit.test.ts`·`TargetPane.test.tsx`·`ComposePhase.test.tsx` 기대값을 바꾸고 test-ledger 에 판단이 바뀐 근거(B-04 · 순회)를 적는다
- [X] T074 [US3] (사유 문구 `Truncate` + 기존 `title` · 머리 줄 이름표 셋 `whitespace-nowrap shrink-0` · 고른 개수도 · 자연어 입력 `min-w-[240px]`) 비활성 사유가 먼저 줄어든다 — `frontend/src/components/workbench/ActionButton.tsx` 사유 문구 `min-w-0` + `Truncate`, `frontend/src/components/workbench/StepList.tsx` Step 패널 머리의 「TEST STEPS」·개수 `shrink-0 whitespace-nowrap`, `frontend/src/components/workbench/ActionPalette.tsx` 자연어 입력 최소 폭 240px (layout-contract-v3 L6 · B-05 · B-06). `PhaseBarWidth`·`AuthoringParity`·`DeleteSelection` 통과
- [X] T075 (알려진 깨짐 46 → **0** · 등록되지 않은 검출 0 · 닿지 못함 0. 전환 중 새로 찾은 N-07 — Step 패널이 460 을 넘어 늘어남 — 을 고쳤고 그 확인에서 자연어 입력 줄의 사유가 「…」만 남는 것을 봐 줄 접힘으로 고쳤다 · 등록부를 비웠다) [US3] 순회를 돌려 남은 검출을 처리한다 — 알려진 깨짐 등록부의 남은 항목을 고치고, 전환 중 새로 찾은 깨짐은 `N-xx` 로 [baseline.md](baseline.md) 뒤 「전환 중 발견」 표에 적은 뒤 고치거나 사유와 함께 허용 등록한다. **알려진 깨짐 등록부 0 · 등록되지 않은 검출 0** (SC-001~SC-003)
- [X] T076 (의도된 차이 106 → 182건 — 뿌리 스크롤 · 기준 폭 자리 · 만들기 대상 앱 자리(B-04) · Step 패널 460(N-07)·머리 줄·바닥 상한·자연어 입력 폭. 대조는 정본 기준 폭 1440 에서 재고, 아트보드가 더한 두 겹은 경로에서 건너뛴다 · 등록되지 않은 차이 0) [US3] L2 `--compare` 를 돌려 배치 수정(B-03·B-04·B-10·B-11)의 차이를 사유와 함께 `scripts/design_compare_ba.py` `INTENDED` 에 등록하고 `frontend/tests/l2-report.json` 을 갱신한다. 등록되지 않은 차이 0 (SC-007)
...
- [X] T077 (동작 층 부품 파일이 전부 있고 `radix-ui` 를 가져오는지 · 조작 부품 파일이 전부 있고 비활성이 점선인지를 파일 존재와 무관하게 요구한다 · 출처 줄은 이미 전 파일에 요구) [US4] G-F 요구를 확정한다 — `frontend/tests/UiSkin.test.ts` 가 모든 `src/ui/*.tsx` 의 출처 줄 · 조작 부품의 `disabled:border-dashed` · `radix-ui` 가져오기 허용 파일 목록([ui-parts.md](contracts/ui-parts.md) §1 `behavior = radix`)을 **파일 존재와 무관하게** 요구한다
- [X] T078 (세 화면은 Foundational 부터 증거와 함께 순회됐다 · 알려진 깨짐 등록부를 비우고 `ScreenSweep` 이 비어 있음을 요구한다) [US4] `scripts/screen_sweep.py` SW-5 화면 목록을 마감한다 — `test-list-row-menu`(Radix 트리거) · `edit-delete-confirm` · `runner-disconnected`(이벤트 소켓 끊기) 가 닿았다는 증거와 함께 순회된다. 알려진 깨짐 등록부 기능은 남기되 비어 있어야 한다는 단언을 `frontend/tests/ScreenSweep.test.ts` 에 더한다
- [X] T079 (8가지 모두 해당 가드가 잡았다 — 충돌 쌍은 두 순서를 넣어 「나중에 적은 쪽이 지는」 순서에서 실패 · quickstart §1-2 실행 기록) [US4] quickstart §1-2 를 실행한다 — 금지 형태 8가지를 하나씩 넣어 실패하는 가드를 확인하고 되돌린다. 결과(가드 이름 · 실패 메시지 첫 줄)를 [quickstart.md](quickstart.md) §1-2 표 옆에 기록한다 (SC-010)
- [X] T080 (CSS gzip 6.69 kB · JS gzip 177.40 kB — spec 상한 안. JS 상한을 +30% 로 올린 판단은 사용자 확인 대상으로 baseline·PENDING §17-1 에 적었다) [US4] 번들을 잰다 — `cd frontend && npx vite build`. CSS gzip ≤ 6.78 kB · JS gzip ≤ 181.3 kB. 결과를 [baseline.md](baseline.md) 「빌드 산출물」 아래 「전환 후」 줄로 적는다. 넘으면 plan RK-5 절차 (SC-011). **Foundational 에서 드러난 것**: Tailwind 자동 소스 탐지가 `frontend/tests/` 까지 훑어 가드 테스트의 금지 예시(`disabled:opacity-50` 등)가 배포 CSS 에 실렸다(26.90 → 28.59 kB). `frontend/src/theme/tailwind.css` 에 `@source not` 으로 `tests`·`scripts` 를 뺀다 — US1 첫 커밋(T021)에서 함께 한다
- [X] T081 (예상 줄 전부 상태를 채웠다 · 단언 2274 ≥ 2148 · 무른 단언 639 = 기준선 · 파일 단위 전후는 줄마다) [US4] [contracts/test-ledger.md](contracts/test-ledger.md) 를 마감한다 — 예상 줄마다 `상태`(고침·불필요) 채우기, `node frontend/scripts/count-assertions.mjs` 로 단언 총수 ≥ 2148 · 무른 단언 증가분의 사유, 파일 단위 전후 수 (SC-008)
...
- [X] T082 [P] (§17 · H-1~H-8 기록 칸 · 사용자 확인이 필요한 판단 여섯) `docs/PENDING-HUMAN-VERIFICATION.md` 에 §17 「017 — shadcn 전환이 모습을 지키고 동작을 표준화했는가」를 더한다 — quickstart H-1~H-8 절차 · 기록 칸 · 닫히는 조건
- [X] T083 [P] `README.md` 「현재 상태」에 017 절을 더한다 — 무엇이 바뀌었나(부품 층 · 알림 자리 · 화면 정책) · 들이지 않은 것 · 검증 방법(순회 · L2 · 가드) 한 단락씩
- [X] T084 [P] 015 계약 문서에 개정 표시를 단다 — `specs/015-tailwind-css-migration/contracts/layout-contract-v2.md` 머리에 「v3 가 L1~L6 을 더했다」 링크, `specs/015-tailwind-css-migration/spec.md` FR-007 에 「017 이 대체」 표시
- [X] T085 (quickstart 끝 「실행 기록」 · 전 항목 기대대로) quickstart §1~§5 전체를 실행한다 — 타입 검사 · 테스트 · 단언 수 · 가드 · 순회 · L2 · 번들 · `git diff --stat 75520ee -- backend/` 0 (원칙 II 증거). 결과를 [quickstart.md](quickstart.md) 끝 「실행 기록」 절에 날짜와 함께 적는다
- [X] T086 (스파이크 잔재 0 · 「초점을 가둔다」 류 낡은 주석 0 · `noUnusedLocals` 로 쓰이지 않는 가져오기 0 · 시각 언어 위반 색 0 · 인라인 30 → 17 · 팔레트 밖 0종) 소스 차이를 훑어 정리한다 — 스파이크 잔재(`__spike_*`) · 쓰이지 않는 가져오기 · 옛 주석(「초점을 가둔다」 등 사실과 달라진 주석) · 이식 중 남긴 원본 클래스. `node frontend/scripts/count-violations.mjs` 가 전환 전(색 0 · 팔레트 밖 0종)보다 나빠지지 않았음을 확인
...
- [X] T104 (**스파이크 여덟 번이 설계를 정했다** — 추측으로 쓴 줄이 없다. ① `Viewport` 는 필수가 아니다(없는 부품을 알리는 오류는 `Root`·`Portal` 둘뿐) → 상세 판은 마디를 안 늘린다. ② **`finalFocus` 기본값이 트리거 없이도 직전 초점으로 돌려준다** → 수제 `useReturnFocus` 삭제. ③ `AlertDialog.Root` 는 `modal`·`disablePointerDismissal` 을 아예 받지 않고 가림막 누름에 `onOpenChange` 조차 안 부른다 → 보호 코드 불필요. ④ 비모달+`disablePointerDismissal` 이 `outside-press` 와 `focus-out` 을 **둘 다** 막는다 → 상세 판의 `onInteractOutside` 가 스위치 하나로. ⑤ **모달은 자기 포털 안에서 시작한 누름만 `outside-press` 로 받는다**(바깥은 `data-base-ui-inert`) → `inNoticeLayer` 가 죽은 코드가 되어 R6 ④ 가 공짜. ⑥ `render={<Button/>}` 이 정본 클래스를 **그대로 지킨다**(기준과 문자열 동일). ⑦ 가림막은 내용을 감쌀 수 없어 `Backdrop`(색)+`Viewport`(가운데·스크롤)로 갈랐다 — 감싸서 얻던 것은 `Viewport` 가 그대로 한다. **판정 방법 셋을 옮겼다**(약화 아님): `data-state="open"`→`data-open` 4곳 · 미러 보호가 `body` 의 `pointer-events:none`→바깥의 `aria-hidden`+`data-base-ui-inert` · Tab 가두기는 jsdom 이 `inert` 를 구현하지 않아 「살아 있는 뒤쪽 조작에 앉지 않는다」로. 가드 G-F 는 갈래를 **파일마다** 보도록 `BASE_PARTS`/`RADIX_PARTS` 로 갈랐다 — 옮기는 동안에도 질문이 살아 있다. 타입 통과 · 1415/1417(남은 둘은 보고서 낡음) · 단언 2274 · 무른 639) `frontend/src/ui/Dialog.tsx` · `AlertDialog.tsx` · `OverlayPane.tsx` 를 이식한다 — Backdrop/Popup · `initialFocus`/`finalFocus`(수제 `useReturnFocus` 삭제) · 바깥 누름은 닫힌 이유로 · 취소 단추는 `render`
- [X] T105 (**부품은 한 번에 옮겨졌고, 시간이 든 곳은 「왜 안 열리나」였다.** 누름으로 메뉴가 안 열려 가설 넷을 세웠고 **넷 다 실측으로 틀렸다**: radix `Tooltip` 감싸개(아님 — 감싼 쪽이 오히려 열렸다) · 제어/토글 처리기(아님 — `onOpenChange` 가 아예 안 불렸다) · 누름 사건의 빠진 칸(아님 — `button=0 buttons=1 pointerType="mouse"` 가 멀쩡했고 손으로 만든 누름도 안 열렸다) · 내 `render={children}` 트리거(아님 — **날것의 Base UI 도 똑같이 안 열렸다**). 진짜 원인은 부품 소스에 있었다: `useClick` 이 `mousedown` 에서 열되 그 열기를 **`requestAnimationFrame` 뒤로 미룬다**(「Wait until focus is set on the element」). `userEvent` 는 마이크로태스크까지만 기다리므로 누른 직후 읽으면 아직 닫혀 있다 — **키보드 길은 rAF 를 거치지 않아** 그쪽 검사 6건이 내내 통과한 것이 힌트였다. 검사 다섯 파일에 「메뉴를 기다린다」를 넣었다(약화 아님 — `EditEntryPoints` 의 「정의 보기가 없다」는 **기다리지 않으면 아직 안 열려서 통과**하던 자리라 오히려 뜻이 생겼다). **N-08 은 검사가 내 고침을 두 번 되돌려 세웠다** — `finalFocus` 로 칸을 가리키기(그 시점에 칸이 없다) · `autoFocus` 에 맡기기(닫히는 순서에 따라 초점이 `body` 로 떨어진다) 둘 다 실패했고, 결국 **칸이 열리는 렌더에서 우리가 옮긴다**(순서에 기대지 않는다). 자리 검사는 radix 내부 선택자 대신 **우리 표식**(`data-slot=menu-positioner`)으로 묻게 바꿨다 — 실측한 사슬은 `BODY > [data-base-ui-portal] > [menu-positioner]{fixed} > [role=menu]` 다. 타입 통과 · 1417/1419(남은 둘은 보고서 낡음) · 단언 2276 · 무른 639) `frontend/src/ui/DropdownMenu.tsx` 를 이식한다 — Positioner 로 자리, Popup `finalFocus` 로 「이름」 칸 초점(N-08 유지). `frontend/src/pages/TestList.tsx` 의 트리거를 `render` 로
- [X] T106 (**부품 넷을 옮기고 `asChild` 를 없앴다 — 소스·검사의 `radix-ui` 참조가 0 이 됐다.** ① `Button` 의 `asChild` 는 **죽은 코드였다**(전수 확인 — 넘기는 호출부가 하나도 없었다). `useRender` 로 다시 만들지 않고 통로째 지웠다 — 이것이 마지막 radix 참조였다. ② `Tabs`: 부품 이름은 `Tab`·`Panel` 이지만 **내보내는 이름을 그대로 둬** 화면 13곳을 건드리지 않았다. 뿌리 `asChild`→`render` 라 `TargetPane` 의 감싸개 `<div>` 를 없앴고, 순회·L2 가 **아무 차이도 못 찾았다**. ③ `ToggleGroup`: 부품의 값이 **배열**이라 문자열 하나로 옮기고 **빈 배열을 무시**해 「늘 하나가 골라져 있다」를 지킨다(S7). 네 모양의 클래스 표는 **한 줄도 안 바뀌었다** — 상태 표식만 `data-[state=on]`→`data-[pressed]`. ④ `Tooltip`: 「툴팁마다 공급자」(T063)를 **그대로 뒀다** — 없는 부품을 알리는 오류가 `Root`·`Portal`·`Positioner` 셋뿐이라 공급자가 필수가 아님을 확인했다(과제문의 「뿌리 공급자 하나」는 근거가 없어 따르지 않았다). **판정 방법 셋을 옮겼다**(약화 아님): `role="radio"`→`button`·`radiogroup`→`group` **30곳** · 탭의 네이티브 `.disabled`→**`aria-disabled`** 4곳(실측 — 누름은 실제로 막힌다: `onValueChange` 0회. 네이티브 `disabled` 를 걸지 않아 초점을 뺏지 않는 방식이라 FR-006·FR-014 와 같은 방향이고, **`console` 탭의 부정 단언도 같은 통로로** 바꿔 헛되이 통과하지 않게 했다) · 툴팁을 `role="tooltip"`→**우리 표식** 3곳. **접근성이 두 군데 내려갔고 숨기지 않았다**: 고르기 묶음 `aria-checked`→`aria-pressed`(「셋 중 하나」를 잃는다) · 툴팁이 `role`·`aria-describedby` 를 잃는다(지금 비용은 0 이나 「툴팁 글은 접근 가능한 이름 안에」가 **떠받치는 전제**가 됐다) — ui-parts §4 와 H-10 에 적었다. 타입 통과 · 1417/1419(남은 둘은 보고서 낡음) · 단언 **2279** · 무른 639 · 순회 76회 검출 0 · L2 34272칸 불일치 0) `frontend/src/ui/Tabs.tsx` · `ToggleGroup.tsx` · `Tooltip.tsx` · `Button.tsx` 를 이식한다 — `data-active`·`data-pressed` · 빈 값 금지(T095) · 뿌리 공급자 하나 · `asChild` 제거. 관련 테스트 판정을 ledger 대로 옮긴다
- [X] T107 (**지웠고 아무것도 깨지지 않았다** — `npm rm radix-ui` 뒤 `package.json`·`node_modules` 에서 사라졌고 소스·검사의 실제 import 는 0(남은 것은 주석과 가드 자신의 정규식뿐). 타입 통과 · **1419/1419** · 순회 76회 검출 0 · L2 34272칸 불일치 0. **가드 문구를 사실에 맞췄다**: 「`radix-ui` 에 기댄다」→「**자기 갈래**에 기댄다」 · 「radix 는 동작 층 부품만 가져온다」→「**하나도 없다**」. 목록 둘(`BASE_PARTS`/`RADIX_PARTS`)은 **비운 채 남겼다** — 다음 갈래 이동도 한 파일씩 일어날 테니 옮기는 도중에도 질문이 살아 있어야 한다. `ImplementationCount` 는 손댈 것이 없었다(`RETIRED_SYMBOLS` 에 radix·`asChild` 항목이 없다). **번들은 늘었다**: 177.41(radix) → 189.40(둘 다) → **207.74 kB**(base 단독). 지우니 오히려 늘었고, research R12 의 전제(「교체는 추가가 아니다」)가 **반증됐다**. 원인은 재지 않았다 — 분석기를 새로 들여야 하는데 의존성 규율이 막는다. **사용자 결정: 문제 삼지 않는다**. 상한을 210 kB 로 다시 잡고 근거를 quickstart §4·research R12 에 남겼다) `radix-ui` 를 지운다 — `npm rm radix-ui` · 소스·테스트에 남은 참조 0 · 가드(`UiSkin`·`ImplementationCount`)와 [guards.md](contracts/guards.md) 요구를 갈래 이름으로 맞춘다

### 10-4 대조와 기록

- [X] T108 (**전부 통과했고, 초록이 「잡을 수 있어서」인지까지 확인했다.** §1-1 `tsc` 통과 · **1419/1419**(연속 2회) · 단언 **2279**(기준 2275) · 무른 639 · 건너뜀 0 · §1-4 정본 0줄 · §2 순회 76회 **검출 0**(허용 28) · §3 L2 **34272칸 불일치 0**(의도된 차이 188) · §4 CSS 6.34/JS **207.74 kB**(≤210) · 금지 의존성 없음(런타임 의존성 넷뿐) · §5 backend 0줄. **SC-016**: 알림 자리를 옛 자리로 되돌리자 `sweep exit 1 · 검출 4` 로 **N-02 가 네 폭 모두에서 그대로 되살아났고**, 되돌리기를 취소하자 `exit 0 · 검출 0` 에 `git diff` 도 비었다 — 이 순회는 실제로 잡는 눈이다. **한 번 보였다가 재현되지 않은 것 하나**: 전량 1회차의 `DeleteOutcome` 「섞여 있으면 둘 다 말한다」 — 단독 3회·전량 2회 모두 통과라 순서 의존 일회성으로 **사실만 남기고 고치지 않았다**) quickstart §1~§5 를 다시 실행한다 — 타입·테스트·단언 수(2275 이상) · 순회(알림 화면 포함 검출 0) · L2(등록되지 않은 차이 0) · 번들(**JS gzip ≤ 210 kB** — T107 에서 실측 207.74 로 다시 잡았다. 옛 상한 177.41 은 갈래 교체가 늘리지 않는다는 전제에서 나온 값이고 그 전제는 반증됐다) · 정본 무변경 · backend 0. 결과를 quickstart 실행 기록과 test-ledger 에 남기고, **알림 자리를 옛 자리로 되돌리면 순회가 실패하는지**(SC-016) 확인한 뒤 되돌린다
