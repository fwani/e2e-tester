...
단계 이름은 [tasks.md](./tasks.md) 의 Phase 번호와 같다. Phase 1(기준선 실측)은 tasks.md 가
갖는다 — 계획이 정하는 것이 아니라 착수 시점에 재는 값이다.

**순서가 의존성이다.** US1 이 삽입 연산과 조작 id 를 세우고, US2 는 그 삽입 지점에서
브라우저 경로를 잇고, US3 은 그 둘을 행으로 내린다.

### Phase 2 — 표와 정본을 먼저 고친다 (US1~US3 공통 전제)
