...
/**
 * 고른 Step 이 없을 때의 같은 조작 (2026-09-11 사용자 보고).
 *
 * **「맨 끝」이 하나의 정당한 자리다.** 그 전까지 이 조작은 Step 을 고르지 않으면
 * 잠겼고, 그래서 「목록 맨 끝에 하나 더」를 할 길이 제품에 없었다 — 마지막 Step 을
 * 고르면 그 **앞**에서 멈추고, 끝까지 실행하면 대화할 수 없는 국면이 된다.
 *
 * 실측에서 「이름 검색 Step 을 마지막에 추가해 달라」는 목표가 이것 때문에 막혔다.
 */
export const OPEN_BROWSER_AT_END = "브라우저 열어 맨 끝에서 멈추기";
...
export const DISABLED_REASON = {
   ...
   */
  /**
   * 대화는 세션 안에서만 산다 (016 R6). 들어가는 길은 둘이고 **위치 기준이 먼저다**
   * (2026-09-11 사용자 보고) — 끼울 Step 을 골라 그 앞에서 멈추면 AI 가 거기에 끼운다.
   */
  NEEDS_SESSION:
    "AI 와 대화하려면 브라우저가 필요합니다. 끼울 Step 을 골라 「브라우저 열어 이 Step 앞에서 멈추기」로 열거나, 구간을 골라 「AI 로 다시 만들기」로 시작하세요",
  /**
  ...
  */
  ...
  O9: "건너뛸 실패가 없습니다",
  /**
   * O14 — 확정되지 않은 교체는 저장되지 않는다 (016 FR-029 · 2026-09-11 사용자 보고).
   *
   * 서버의 거절 문장(`sessions.py` `save`)과 **같은 사실**을 말한다 — 눌러서 받는
   * 409 의 문장과 미리 보는 사유가 다르면 사용자는 둘이 다른 문제인지 확인하느라 멈춘다.
   * 양쪽 길을 다 적는다: 해소 링크는 확정 하나를 가리키지만, 버리기도 정당한 선택이다.
   */
  O14: "교체가 끝나지 않아 저장할 수 없습니다. 「확정」으로 끝내거나 「버리기」로 되돌리세요",
