...
```bash
cd backend && uv run pytest tests/abnormal/test_catalogue.py -v
```
...
```bash
cd backend && uv run pytest tests/abnormal/test_error_contract.py -v
```
...
```bash
cd backend && uv run pytest tests/abnormal/test_route_sweep.py -v
```
...
```bash
cd backend && uv run pytest tests/abnormal/test_no_bypass.py -v
```
...
```bash
cd backend && uv run pytest tests/abnormal/test_error_contract.py -k internal -v
```
...
```bash
cd backend
uv run pytest tests/abnormal/test_invalid_input.py -v      # AP-010~015
uv run pytest tests/abnormal/test_order_violation.py -v    # AP-020~024
uv run pytest tests/abnormal/test_external_failure.py -v   # AP-030~033
uv run pytest tests/abnormal/test_concurrency.py -v        # AP-040~042
```
...
```bash
cd backend && uv run pytest tests/abnormal/test_atomic_write.py -v
```
...
## 8. 화면 쪽 검증 (AP-003·RG-104-4)

```bash
cd frontend && npm run test -- abnormal
```

**기대**:

- 각 화면의 오류 경로가 공용 오류 통로를 지난다
- 공용 통로가 `message` 와 `next_action` 을 **함께** 보여준다
- 버튼 연타가 요청을 두 번 내보내지 않거나, 두 번째가 거절되고 그 사실이 화면에 드러난다
- 계약 형태가 아닌 응답(연결 실패)에서도 다음 행동이 표시된다
...
```bash
cd backend && uv run pytest tests/abnormal/ -v --tb=short
```
...
- **다른 세션이 같은 저장소를 동시에 수정 중이다** (2026-09-04 13:51 커밋 `725a9bb`). 구현 착수 전에 `git status` 와 최근 커밋을 확인하고, 충돌 가능성이 있으면 조율해야 한다
