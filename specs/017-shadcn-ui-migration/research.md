...
>
> **2026-09-16 재개정 (T107) — 위 전제가 틀렸다. 실측이 반증했다.**
> 같은 명령(`npx vite build`)으로 잰 세 수치: **177.41**(radix) → 189.40(radix + base 둘 다) →
> **207.74 kB**(base 단독). **`radix-ui` 를 지웠는데 오히려 늘었다** — 중간값이 더 작았던 것은 그때
> Base UI 를 절반만 쓰고 있었기 때문이지 radix 가 무거워서가 아니다. 즉 **기반 교체는 이 저장소에서
> 「추가가 아니었다」가 성립하지 않았고**, 약 30 kB 가 늘었다.
>
> **원인은 재지 않았다.** 무엇이 30 kB 인지 보려면 번들 분석기를 새로 들여야 하는데 이 계획은 들이는
> 의존성을 R2·R5 에서 규율로 막아 두었다. 짐작(Base UI 가 floating-ui 를 안고 있다)은 **근거로 쓰지
> 않는다** — `node_modules` 디스크 크기는 번들 바이트의 증거가 아니다.
>
> **사용자 결정 (2026-09-16): 이 정도 증가는 문제 삼지 않는다 — 그대로 둔다.** 그래서 상한을 실측에
> 여유를 둔 **210 kB** 로 다시 잡았다(quickstart §4). 그 숫자는 「옳다고 판정된 값」이 아니라 **「지금
> 이만큼이다」라는 기록**이며, 다음에 여기서 더 늘면 그때는 이유를 물어야 한다.
