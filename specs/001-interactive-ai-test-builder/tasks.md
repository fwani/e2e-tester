...
- [ ] T167 목록에서 테스트 이름 변경·삭제를 할 수 있게 한다 per FR-007 (partial) — **HIGH**. `frontend/src/api/client.ts` 의 `tests.rename`·`tests.remove` 가 정의만 되어 있고 어디서도 호출되지 않는다. `TestList.tsx` 에 두 조작을 붙이고, **삭제는 되돌릴 수 없으므로 확인을 받는다** (FR-007). 확인 없이 지우면 사용자가 만든 자산이 한 번의 오클릭으로 사라진다
- [ ] T168 중지 전에 저장하지 않은 편집의 저장 여부를 확인한다 per FR-042 (partial) — **HIGH**. 서버는 이미 `POST .../stop` 응답과 세션 뷰에 `has_unsaved_changes` 를 실어 보내지만(T098) 프론트가 읽지 않는다. `Runner.tsx` 의 중지가 확인 없이 세션을 끝내므로, 일시정지 중 편집한 내용이 그대로 사라진다. 확인 시점은 **중지를 누른 직후**여야 한다 — 종료 후에는 저장할 대상이 없다
- [ ] T169 저장된 테스트의 정의를 실행하지 않고 볼 수 있게 한다 per FR-016·FR-019 (partial) — **HIGH**. 지금은 Step 목록과 상세를 보려면 재실행 세션을 시작해야 한다. `tests.get` 이 어디서도 쓰이지 않는 것이 그 증거다. 목록에서 테스트를 열어 Step 목록과 후보 우선순위 표(`LocatorPriorityTable`)를 보는 화면을 만든다. **"다시 집기"(FR-020)는 살아 있는 브라우저를 요구하므로 이 화면에 넣지 않는다** — 일시정지 세션에 남기고, 여기서는 보기와 이름·값 편집까지만 한다
- [ ] T170 결과 화면에서 실패한 Step 상세로 이동할 수 있게 한다 per FR-056 (partial) — MEDIUM. `RunResult.tsx` 는 `onEditStep` 을 이미 받지만 `App.tsx` 가 넘기지 않아 버튼이 그려지지 않는다. T169 의 정의 보기 화면이 생기면 그곳으로 연결한다 — 두 작업은 같은 목적지를 공유한다
