...
DEFAULT_SUMMARY_BUDGET = 8192
"""요약 문자열의 바이트 상한 (016 T004 잠정값 · T064 가 실측으로 교체한다).

**너무 작으면** 평범한 테스트에서 축약이 상시로 일어나 에이전트가 늘 부분만 본다.
**너무 크면** 매 턴의 토큰이 낭비된다 — 요약은 대화마다 다시 붙기 때문이다 (FR-003).

한 Step 이 한 줄이고 줄 하나가 60~80바이트이므로 8KB 는 Step 100개를 넉넉히 담는다.
`test_the_default_budget_fits_a_hundred_steps` 가 그것을 고정한다.
"""
