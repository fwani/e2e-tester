...
**릴리스 게이트 등록** (헌법 Incremental delivery 요건 2): **새 게이트 항목을 만들지
않는다.** 이 이연은 001 plan.md 의 릴리스 게이트 표에 이미 등록된 "원칙 V — Playwright
Export 미구현" 항목과 **같은 항목**이다. 004 는 그 항목의 회수 조건에 한 줄을 더한다 —
내보내기를 구현할 때 다후보 대기의 표준 Playwright 표현을 함께 설계해야 한다. 별도
항목으로 등록하면 같은 미구현이 두 게이트로 세어져 회수 판정이 흐려진다.
