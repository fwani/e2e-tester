...
- [X] T022 [US4] [US5] `backend/tests/abnormal/drivers/boundary_drivers.py` 와 `test_boundary_surface.py` — 외부 경계 **14건**(AS-013~017·030~032·040~042·049~051)의 수단을 등록한다. T010·T011 의 대역을 쓴다
- [X] T023 [US2] [US3] [US4] [US5] `backend/tests/abnormal/drivers/ui_drivers.py` 와 `test_ui_surface.py` — 화면 **18건**(AS-007~012·024~029·037~039·046~048)의 수단을 등록한다. **T012 의 실브라우저 픽스처로 제품 화면을 실제로 조작한다.** 버튼 연타가 요청을 몇 번 내보내는지, 화면을 벗어났다 돌아오면 무엇이 보이는지, 두 창에서 조작하면 어떻게 되는지는 여기서만 판정된다 (RG-105)

**외부 경계 14건 전건 통과** (T022·T024·T028). 대상 사이트가 이상하게 굴 때 제품이 어떻게
되는지는 그동안 잰 적이 없었다. 이 과정에서 드러난 제품 결함 5건을 고쳤다.

| 결함 | 무엇이 잘못됐나 | 근거 |
|---|---|---|
| Step 실패에 **분류가 없었다** | 실패 문장 하나만 나가서, 받는 쪽은 "대상 사이트가 응답하지 않은 것"과 "제품이 깨진 것"을 문구로 짐작해야 했다 | AP-033 |
| 대상 사이트 무응답이 **요소 탐색 실패처럼** 보였다 | "대상 요소가 나타나지 않았거나 동작이 막혔습니다" — 사용자를 없는 문제로 보낸다 | AP-031 |
| AI 실패가 **200 응답에 문장으로만** 실렸다 | `created:false` 와 문장뿐. 분류도 다음 행동도 없다 | AP-032·AP-033 |
| 테스트 정의·비밀 저장소가 **곧바로 덮어써졌다** | 쓰기가 끊기면 원본 자리에 반쪽 파일이 남는다. 봉인된 값은 되살릴 방법이 없다 | AP-042 |
| 파일 쓰기 실패가 `INTERNAL_ERROR`(broken) 로 나갔다 | 공간을 비우거나 권한을 고치면 되는데 "할 수 있는 일이 없다"로 읽힌다 | AP-042 |

수단이 분류를 지어내지 않는지는 `test_step_failure_reaches_the_screen_classified` 가 실제
재실행으로 확인한다 — 그 검증이 없으면 판정축 ①이 자기 자신을 재게 된다.
...
- [X] T026 [US2] [US3] T021 이 낸 요청 경계 실패를 고친다 — 경계 검증 누락, 경로 탈출·중복 이름 미거부(AP-013·014), 거부 메시지가 원본 입력을 되돌려 화면을 훼손하는 것(AP-015), 거부가 세션을 못 쓰게 만드는 것(AP-024)
- [X] T027 [US2] [US3] [US5] T023 이 낸 화면 실패를 고친다 — 연타로 요청이 두 번 나가는 것(AP-022), 화면 복귀 시 옛 상태로 멈춘 것(AP-023·AS-026), 반영되지 않은 조작이 조용히 무시되는 것(AP-041)
...
**화면 18건 전건 통과** (T023·T027). 이 저장소가 **제품 화면을 실제 브라우저로 띄운 첫
라운드**다. 이 과정에서 드러난 제품 결함 2건을 고쳤고, 제품과 어긋난 시나리오 정의 4건을
바로잡았다 (근거는 목록의 `note`).

| 결함 | 무엇이 잘못됐나 | 근거 |
|---|---|---|
| 프로젝트 「만들기」가 **이유 없이** 눌리지 않았다 | 무엇이 빠졌는지 화면에 없었다. 키 관리 화면은 같은 자리에서 규칙을 알리고 있었다 | AP-003 |
| **AI 실패 사유가 화면에서 사라졌다** | 실패한 세션이 검토를 위해 `paused` 로 옮겨가면 사유를 그리는 유일한 화면을 떠난다. 001 의 같은 결함을 002 가 고쳤으나 구멍이 `paused` 로 옮겨졌을 뿐이었다 | AP-032·DR-020 |

판정축에 **`prevented`** 를 더했다 — 화면이 요청을 아예 보내지 못하게 막은 경우다. 보내고
거절하는 것보다 나은 설계이므로 그 자리에 오류 본문을 요구하면 더 나은 것을 결함으로
판정하게 된다. 면제가 아니다: 무엇이 막는지 화면에 보이지 않으면 판정축 ②가 잡는다.

---

## Phase 7: 회귀와 마감

- [X] T029 [US6] 전체 회귀 — `cd backend && uv run python -m pytest` 와 `cd frontend && npm run test -- --run && npm run typecheck` 가 실패 0건. 실패하면 검증이 아니라 구현을 고친다 (헌법 게이트 4·RG-102·SC-207)
- [X] T030 [US6] 스키마 드리프트 확인 — `uv run python -m itb.schema.export --check` 와 `npm run gen:types && git diff --exit-code src/types/generated/` (EC-006)
- [X] T031 [US6] 재실행 경로에 LLM 호출이 도달하지 않음을 확인한다. 이 라운드가 AI 대역을 도입했으므로 그것이 **검증 코드에만** 있다는 증거를 남긴다 (헌법 원칙 II·게이트 1)
- [X] T032 판정 결과를 집계해 `SC-201`~`SC-211` 각각의 충족 여부를 `specs/003-error-path-hardening/` 아래 결과 문서에 기록한다. 미달 항목은 이유와 함께 남긴다
- [X] T033 [P] `quickstart.md` 의 10단계를 실제로 돌려 문서와 실제가 어긋나지 않는지 확인하고, `README.md` 의 "현재 상태" 표에 003 을 더한다. **릴리스 게이트 RG-1(Playwright Export)이 여전히 미완임을 흐리지 않는다**
- [X] T034 Edge Case 둘을 확인한다 — 이상 조작을 연달아 반복해도 자원이 누적되지 않는가, 오류 처리 자체가 실패하면 사용자가 무엇을 보는가
