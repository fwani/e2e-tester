...
**민감 값 인계**는 값을 옮기지 않고 **요구를 옮긴다.** 묶음에는 민감 변수의 이름과 쓰이는 자리만
있다. 가져오기 결과가 그 목록을 「어느 테스트의 어느 스텝에서 쓰인다」와 함께 보여 주고, 받은
사람이 그 자리에서 값을 채우면 **자기 키로** 봉인된다 — 봉인은 기존 `PUT /api/secrets/{name}` 이
하던 일 그대로다. 값이 비면 세션 생성 시점에 막힌다.
...
| 모든 외부 입력은 경계에서 검증 | 묶음 파일은 ① 바이트 상한 ② YAML 별칭 금지 로더 ③ Pydantic 모델 ④ `Test` 도메인 검증 의 네 겹을 통과해야 한다. |
...
│   ├── sharing/                          # 신규 패키지
│   │   ├── __init__.py
│   │   ├── bundle.py                     # 묶음 모델 (ShareBundle, BundleMeta, RequiredSecret)
