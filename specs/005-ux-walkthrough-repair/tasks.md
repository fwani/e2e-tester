...
- [ ] T112 [US2] 재실행 세션 제목의 「초안」을 없앤다 — **U-03-a** (FR-134 미달). 저장 경로(T079)만 고쳐졌고 재실행 세션은 여전히 「TC-001 초안」이다. 같은 화면 부품이 저장 세션에서는 「TC-003 · 저장됨」을 쓴다
- [ ] T113 [US2] 중지 후 화면에서 저장 프롬프트(「테스트 이름」 + 「저장」)를 걷고 ui-contract §8 의 네 번째 다음 행동 「결과 자세히 보기」를 붙인다 — **U-03-a** (FR-133, 현재 3개)
- [ ] T114 [US2] 종료된 세션에 대한 `GET /api/sessions/{id}/tabs` 폴링을 멈추고, 그 404 를 오류 배너로 띄우지 않는다. 배너 문구에서 세션 UUID 를 뺀다 — **U-03-b** (FR-135 미달, 실측 404 3건)
- [ ] T115 [US3] 실행이 끝나면 일시정지 전이 배너(「…아직 실행 중입니다」)를 지운다 — **U-04-a** (FR-146 의 "낡은 전이 안내를 지운다" 미이행)
- [ ] T116 [US3] 미러 오버레이 문구를 실제 상태와 맞춘다 — 전이 중·실행 종료 후에도 「일시정지 · 브라우저 세션과 화면 상태를 그대로 유지하고 있습니다」로 단정한다. 배지와 같은 화면에서 다른 말을 한다 — **U-04-b** (FR-142·FR-146)
- [ ] T117 [US3] 「멈추기 전에 실행이 끝났습니다」 화면에 실패 사유와 「결과 자세히 보기」를 붙인다 — **U-04-c** (FR-146, 요약 한 줄만 있어 진단 경로를 잃는다)

### 재점검이 새로 찾은 것 — 005 요구사항에 걸리는 것만

- [ ] T118 [US7] 새로고침이 화면 상태를 잃는다 — 결과 화면(`?screen=result&test=…`)에서 F5 하면 목록으로 떨어지고 주소가 `/` 로 초기화된다. **FR-166·FR-167 미충족** (T095·T096 은 뒤로가기 이탈 방지만 이행했다). 뒤로가기는 통과 — **N-01**
- [ ] T119 [US7] 목록 행 배지가 저장된 결말이 아니라 **열린 세션의 존재**를 본다 — 실행이 끝나도 행은 `RUNNING`·「실행 중」에 고착되고, 같은 화면의 배너는 「실패」로 갱신된다. 배너와 행이 다른 말을 한다 — **N-02** (FR-168·FR-169)
- [ ] T120 [US8] 통과한 실행의 SCREENSHOT 탭이 「불러올 수 없습니다 / 산출물이 지워졌을 수 있습니다」로 말한다. 통과에는 실패 시점 스크린샷이 없는 것이 정상이므로 없다는 사실을 **긍정문**으로 말한다 — **N-03** (FR-173 미달)
- [ ] T121 [US2] 「실패한 Step 건너뛰고 계속」 컨트롤이 화면에 없다 — 백엔드·계약에는 `skip_failed` 가 있으나(`runner.py`·rest-api.md §3-b) 프론트에서 보내는 곳이 0건이라 `partial_pass` 를 UI 로 만들 수 없다. quickstart S3-6 이 확인 불가로 남았다 — **N-04** (FR-136·FR-137 의 화면 절반). **의도적으로 뺀 것인지 먼저 확인한다**
- [ ] T122 [P] 녹화 일시정지 부제가 없는 Step 번호를 말한다 — Step 5개를 녹화하고 멈추면 「Step 06 이후 정지」다. `RunnerPaused.tsx:274` 가 `stepLabel(currentStepIndex)` 에 **다음 위치**를 넘긴다. `stepLabel` 은 옳고 인자가 틀렸다 — **N-06** (FR-138)
